...
(function () {
  ...
  async function waitForSave(status, before) {
    for (let i = 0; i < 100; i += 1) {
      ...
      if (text !== before && /^(Gespeichert|Keine Änderungen)/i.test(text)) return true;
  ...
  async function uploadTemplate(file, status) {
    ...
    if (status) status.innerHTML = `<strong>LG-Vorlage gespeichert ✓</strong> ${body.template?.skuCount || ""} SKU-Zeilen erkannt. Diese Vorlage bleibt auf KRISTINE gespeichert.`;
    return true;
  }

  async function ensureTemplate(status) {
  ...
  async function createExcel(button) {
    ...
    try {
      ...
      if (!(await ensureTemplate(status))) {
        if (status) status.innerHTML = "<strong>Einmalig:</strong> Bitte deine aktuelle Little-Greene-Bestellvorlage auswählen. Sie bleibt danach gespeichert.";
      ...
      } else if (Array.isArray(body.missing) && body.missing.length) detail += ` · fehlende SKU: ${body.missing[0].sku}`;
  ...
  function install() {
    ...
    button.id = "lgOrderExcel"; button.className = "btn primary"; button.type = "button"; button.textContent = "LG-Excel erstellen";
