...
(function () {
  ...
  style.textContent = `
    ...
    .return-search{margin-top:10px}.return-results{margin-top:12px;display:grid;gap:12px}
    .return-row{min-width:0;border:1px solid var(--line);border-radius:14px;background:#fff;padding:16px;box-shadow:0 2px 8px #00000008}
    .return-row-head{display:flex;justify-content:space-between;align-items:baseline;gap:12px}.return-no{overflow-wrap:anywhere;font-size:25px;font-weight:900;line-height:1.2}.return-weight{font-size:22px;font-weight:900;white-space:nowrap}
    .return-main{min-width:0;margin-top:10px}.return-main b{display:block;font-size:18px;line-height:1.35;overflow-wrap:anywhere}.return-sub{font-size:14px;color:#4c5550;margin-top:5px;line-height:1.45;overflow-wrap:anywhere}.return-project-name{display:block;margin-top:4px}.return-state{display:inline-block;color:#92322c;font-size:14px;margin-top:6px}
    .return-row-foot{display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:10px;margin-top:14px;padding-top:12px;border-top:1px solid var(--line)}.return-age{font-size:13px;color:#4c5550}.return-actions{display:flex;flex-wrap:wrap;gap:7px}.return-actions .btn{min-height:38px;white-space:nowrap}
    ...
    @media(max-width:760px){.return-layout{grid-template-columns:1fr}.return-project{align-items:flex-start}.return-project-value{font-size:16px}.return-scan-actions{display:grid;grid-template-columns:1fr}.return-scan-actions .btn,.return-ean{width:100%;max-width:none}.return-fields,.return-learn-grid{grid-template-columns:1fr}.return-row{padding:14px}.return-no{font-size:23px}.return-weight{font-size:21px}.return-row-foot{align-items:flex-start}.return-actions{width:100%}}
  `;
  ...
  function renderReturns(items) {
    ...
    el("returnResults").innerHTML = returns.map((row) => {
      ...
      return `<div class="return-row"><div class="return-row-head"><div class="return-no">${esc(row.returnLabel || row.returnNo)}</div><div class="return-weight">${esc(formatWeight(row.weightKg))}</div></div><div class="return-main"><b>${esc(row.colour)}</b><div class="return-sub">${esc(material)}${projectText ? `<span class="return-project-name">Baustelle: ${esc(projectText)}</span>` : ""}</div>${row.status !== "available" ? `<strong class="return-state">${esc(statusLabel(row.status))}</strong>` : ""}</div><div class="return-row-foot"><span class="return-age">${esc(ageText(row.ageDays))}</span><div class="return-actions"><button class="btn" type="button" data-return-edit="${esc(row.id)}">${row.status === "available" ? "Ändern" : "Verlauf"}</button>${["available","out"].includes(row.status) ? `<button class="btn" type="button" data-return-transfer="${esc(row.id)}">${row.status === "out" ? "Zurückbuchen" : "Ausbuchen"}</button>` : ""}${row.status === "available" ? `<button class="btn" type="button" style="color:#a7322d" data-return-remove="${esc(row.id)}">Entfernen</button>` : ""}</div></div></div>`;
    }).join("");
