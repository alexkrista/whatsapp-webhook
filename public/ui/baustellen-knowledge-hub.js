...
(function(){
  ...
  function masterPersonCard(title,person){
    const p=person||{},name=[p.firstName,p.lastName].filter(Boolean).join(" "),has=!!(p.company||name||p.phone||p.email||(p.extraLines||[]).length);
  ...
  function renderMasterData(j){
    ...
    const el=document.getElementById("bkMasterData");el.className="";el.innerHTML=`<div class="bk-grid">
      <div class="bk-card bk-half"><div class="bk-section-title"><h3>Baustelle</h3>${masterStatus}</div><dl class="bk-master"><dt>Baustellennummer</dt><dd>${esc(j.jobId||'–')}</dd><dt>Bezeichnung</dt><dd>${esc(j.name||'–')}</dd><dt>Status</dt><dd>${esc(j.status||'–')}</dd><dt>Baustellenadresse</dt><dd>${esc(siteAddress)}</dd><dt>Adresszusatz / Zufahrt</dt><dd>${esc(j.addressExtra||'–')}</dd></dl></div>
  ...
  function renderMasterDataEditor(j){
    ...
    const personFields=(prefix,p)=>`${field("Firma / Büro",prefix+"Company",p.company,true)}${field("Vorname",prefix+"FirstName",p.firstName)}${field("Nachname",prefix+"LastName",p.lastName)}${field("Telefon",prefix+"Phone",p.phone)}${field("E-Mail",prefix+"Email",p.email,false,"email")}`;
