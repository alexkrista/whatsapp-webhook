...
(function(){
  const VERSION="2026-08-22-knowledge-1";
  ...
  function renderProtocols(j,days){
    ...
    const rows=days.length?days.map(d=>`<div class="bk-day"><strong>${fmtDate(d.day)}</strong><div class="bk-day-meta">🖼 ${num(d.stats?.images)} Fotos · 📄 ${num(d.stats?.pdfs)} PDFs · 🎤 ${num(d.stats?.audio)} Audio · ${num(d.stats?.items)} Einträge</div><div class="bk-actions"><button type="button" class="primary" data-bk-preview="${esc(d.viewUrl||`/admin/pdf/${encodeURIComponent(j.jobId)}/${encodeURIComponent(d.day)}`)}" data-bk-label="${esc('Bautag '+fmtDate(d.day))}">Fotos & Protokoll ansehen</button><a href="${tokenUrl(d.downloadUrl||`/admin/download/${encodeURIComponent(j.jobId)}/${encodeURIComponent(d.day)}`)}">PDF</a></div></div>`).join(''):'<div class="bk-placeholder">Noch keine Bautage dokumentiert.</div>';
    const el=document.getElementById("bkProtocols");el.className="";el.innerHTML=`<div class="bk-grid"><div class="bk-card"><div class="bk-label">Bautage</div><div class="bk-value">${days.length}</div></div><div class="bk-card"><div class="bk-label">Fotos</div><div class="bk-value">${totals.images}</div></div><div class="bk-card"><div class="bk-label">Dokumente/PDF</div><div class="bk-value">${totals.pdfs}</div></div><div class="bk-card"><div class="bk-label">Audio</div><div class="bk-value">${totals.audio}</div></div><div class="bk-card bk-wide"><div class="bk-section-title"><h3>Bautage · Fotos · Protokolle</h3><span class="bk-source">Baustellenakte</span></div><div class="bk-day-list">${rows}</div><div id="bkProtocolPreview"></div></div></div>`;
