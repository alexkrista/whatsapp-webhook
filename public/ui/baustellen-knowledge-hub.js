...
(function(){
  ...
  async function api(p,o={}){if(o.method==="PUT"&&/\/admin\/api\/job\/[^/]+\/meta$/.test(p)&&o.body){const payload=JSON.parse(o.body),pc=payload.projectContacts||{},v=id=>document.getElementById(id)?.value?.trim()||"";pc.owner={...(pc.owner||{}),sharedLastName:v("bkOwnerLastName")||pc.owner?.customer||"",womanTitle:v("bkOwnerWomanTitle"),womanFirstName:v("bkOwnerWomanFirstName")||pc.owner?.firstName||"",manTitle:v("bkOwnerManTitle"),manFirstName:v("bkOwnerManFirstName")};pc.siteManager={...(pc.siteManager||{}),title:v("bkSiteManagerTitle"),alsoArchitect:!!document.getElementById("bkSiteManagerAlsoArchitect")?.checked};pc.architect={...(pc.architect||{}),title:v("bkArchitectTitle")};pc.deliveryRecipients={};for(const doc of ["offer","order","regie","invoice"]){pc.deliveryRecipients[doc]={};for(const group of ["owner","siteManager","architect"])pc.deliveryRecipients[doc][group]=!!document.querySelector(`[data-delivery-doc="${doc}"][data-delivery-group="${group}"]`)?.checked}payload.projectContacts=pc;const names=[[pc.owner.womanTitle,pc.owner.womanFirstName,pc.owner.sharedLastName],[pc.owner.manTitle,pc.owner.manFirstName,pc.owner.sharedLastName]].map(x=>x.filter(Boolean).join(" ")).filter(Boolean);payload.contactName=names.join(" und ")||payload.contactName;o={...o,body:JSON.stringify(payload)}}const r=await fetch(tokenUrl(p),o);const t=await r.text();let d;try{d=JSON.parse(t)}catch{}if(!r.ok)throw new Error(d?.error||t||r.statusText);return d}
  ...
  function renderMasterData(j){
    ...
    const el=document.getElementById("bkMasterData");el.className="";el.innerHTML=`<div class="bk-grid">
      ...
      <div class="bk-card bk-half"><div class="bk-section-title"><h3>Bauherrschaft</h3><span class="bk-badge blue">${esc(owner.ownerRole||'Bauherrschaft')}</span></div><dl class="bk-master"><dt>Kunde / Nachname / Firma</dt><dd>${esc(owner.customer||m.name||j.contactName||'–')}</dd><dt>Vorname</dt><dd>${esc(owner.firstName||'–')}</dd><dt>Wohnadresse</dt><dd>${esc(residence)}</dd><dt>Telefon Bauherrin</dt><dd>${masterPhone(owner.phoneOwnerWoman)}</dd><dt>Telefon Bauherr</dt><dd>${masterPhone(owner.phoneOwnerMan||m.phone||j.contactPhone)}</dd><dt>E-Mail Bauherrschaft</dt><dd>${masterEmail(owner.email||m.email||j.contactEmail)}</dd>${masterExtraRows(owner.extraLines)}<dt>WW-Kundennummer</dt><dd>${esc(wwNumber||'–')}</dd><dt>WW-Stammindex</dt><dd>${esc(wwId||'–')}</dd></dl></div>
    ...
    document.getElementById("bkOwnerFirstName")?.closest("label")?.insertAdjacentHTML("beforebegin",field("Gemeinsamer Nachname","bkOwnerLastName",owner.sharedLastName||owner.customer,true)+field("Titel Bauherrin","bkOwnerWomanTitle",owner.womanTitle)+field("Vorname Bauherrin","bkOwnerWomanFirstName",owner.womanFirstName||owner.firstName)+field("Titel Bauherr","bkOwnerManTitle",owner.manTitle)+field("Vorname Bauherr","bkOwnerManFirstName",owner.manFirstName));
    document.getElementById("bkOwnerFirstName")?.closest("label")?.remove();
  ...
  function renderMasterDataEditor(j){
    ...
    const extras={owner:[...(owner.extraLines||[])],siteManager:[...(siteManager.extraLines||[])],architect:[...(architect.extraLines||[])]};
    ...
    document.getElementById("bkSaveMaster").onclick=async()=>{const button=document.getElementById("bkSaveMaster"),status=document.getElementById("bkMasterSaveStatus"),projectContacts={owner:{customer:val("bkOwnerCustomer"),firstName:val("bkOwnerFirstName"),ownerRole:val("bkOwnerRole"),residentialStreet:val("bkHomeStreet"),residentialHouseNumber:val("bkHomeHouse"),residentialPostalCode:val("bkHomePostal"),residentialCity:val("bkHomeCity"),phoneOwnerWoman:val("bkOwnerWomanPhone"),phoneOwnerMan:val("bkOwnerManPhone"),email:val("bkOwnerEmail"),extraLines:cleanExtras("owner")},siteManager:{company:val("bkSiteManagerCompany"),firstName:val("bkSiteManagerFirstName"),lastName:val("bkSiteManagerLastName"),phone:val("bkSiteManagerPhone"),email:val("bkSiteManagerEmail"),extraLines:cleanExtras("siteManager")},architect:{company:val("bkArchitectCompany"),firstName:val("bkArchitectFirstName"),lastName:val("bkArchitectLastName"),phone:val("bkArchitectPhone"),email:val("bkArchitectEmail"),extraLines:cleanExtras("architect")}};button.disabled=true;status.textContent="Stammdaten werden gespeichert …";try{const ownerName=[projectContacts.owner.firstName,projectContacts.owner.customer].filter(Boolean).join(" "),payload={name:val("bkJobName"),street:val("bkSiteStreet"),houseNumber:val("bkSiteHouse"),postalCode:val("bkSitePostal"),city:val("bkSiteCity"),addressExtra:val("bkSiteExtra"),contactName:ownerName,contactPhone:projectContacts.owner.phoneOwnerWoman||projectContacts.owner.phoneOwnerMan,contactEmail:projectContacts.owner.email,projectContacts},result=await api(`/admin/api/job/${encodeURIComponent(j.jobId)}/meta`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});Object.assign(j,result.meta||{});const cached=jobById(j.jobId);if(cached&&cached!==j)Object.assign(cached,result.meta||{});renderMasterData(j)}catch(error){button.disabled=false;status.textContent="Speichern nicht möglich: "+error.message}};
  }
