...
(function(){
  const VERSION="2026-09-03-live-hours-3";
  ...
  function renderHoursReconciliation(j){
    ...
    card.querySelectorAll("[data-hr-key]").forEach(input=>input.addEventListener("change",()=>{const draft=new Set(selected);if(input.checked)draft.add(input.dataset.hrKey);else draft.delete(input.dataset.hrKey);reconciliationDrafts.set(jobId,draft);patchAll()}));
    card.querySelector("#hrSave")?.addEventListener("click",async event=>{const button=event.currentTarget,status=card.querySelector("#hrStatus"),draft=reconciliationDrafts.get(jobId)||new Set(selected);button.disabled=true;status.textContent="Auswahl wird gespeichert …";try{const response=await apiWrite(`/admin/api/job/${encodeURIComponent(jobId)}/hours-overlap`,{excludedWwKeys:[...draft]});j.hoursCutoverDate="";j.hoursOverlapExcludedWwKeys=response.excludedWwKeys||[];j.hoursOverlapResolvedAt=response.resolvedAt||new Date().toISOString();reconciliationDrafts.set(jobId,new Set(j.hoursOverlapExcludedWwKeys));card.dataset.signature="";patchAll()}catch(e){button.disabled=false;status.textContent="Speichern nicht möglich: "+e.message}});
  }
