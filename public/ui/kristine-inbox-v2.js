...
`;
  ...
  function showAnalysis(item){
    ...
    document.getElementById("kristaInboxContent").innerHTML=`<div class="krista-inbox-reco"><strong>KRISTINE empfiehlt: ${esc(ROUTES[rec]||"Ablage")}</strong> · ${pct}%<br><span class="small">${esc((a.reasons||[]).join(" · ")||"Inhalt wurde analysiert.")}</span></div><div class="krista-inbox-meta"><div><strong>Betreff</strong><br>${esc(a.subject||"–")}</div><div><strong>Fällig/Termin</strong><br>${esc(a.dueDate||"–")}</div><div><strong>Kontakt</strong><br>${esc(a.contactName||"–")}</div><div><strong>Telefon</strong><br>${esc(a.contactPhone||"–")}</div><div><strong>E-Mail</strong><br>${esc(a.contactEmail||"–")}</div><div><strong>Erkannt</strong><br>${esc(a.product||a.colorName||"–")}</div></div>${attachments?`<div class="krista-inbox-pending"><strong>Anlagen</strong><br>${attachments}</div>`:""}<div class="krista-inbox-assignee"><label><strong>Aufgabe für</strong></label><select id="kristaInboxAssignee">${assigneeOptionsHtml()}</select><div class="small" style="margin-top:5px">Standard: Alexander Krista · bei Bedarf ändern.</div></div><div class="krista-inbox-routes"><button type="button" onclick="return window.KristineInboxV2.dismiss()">${item.status==="dismissed"?"Wiederherstellen":"Verwerfen"}</button>${Object.keys(ROUTES).map(r=>`<button type="button" class="${r===rec?'recommended':''}" onclick="return window.KristineInboxV2.route('${r}',event)">${ROUTES[r]}</button>`).join("")}</div><div id="kristaInboxRouteStatus" class="krista-inbox-route-status"></div>${a.excerpt?`<div class="krista-inbox-preview">${esc(a.excerpt)}</div>`:""}`;
    document.getElementById("kristaInboxModalBg").classList.add("open");
  }
  ...
  async function dismiss(){
    if(!current||routing)return false;
    routing=true;disableRoutes(true);
    try{const result=await api(`/kristine/api/inbox/${encodeURIComponent(current.id)}/dismiss`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({restore:current.status==="dismissed"})});showAnalysis(result.item);routeStatus(current.status==="dismissed"?"Verworfen. Originalmail bleibt erhalten. Wiederherstellen ist jederzeit möglich.":"Eintrag wiederhergestellt.")}catch(error){routeStatus(error.message,true)}finally{routing=false;disableRoutes(false)}
    return false;
  }
  ...
  window.KristineInboxV2={route,dismiss,version:VERSION};
