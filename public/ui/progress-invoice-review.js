...
(function(){
  ...
  function open(p){
    ...
    dialog.innerHTML='<form method="dialog"><button style="float:right" aria-label="Schließen">×</button></form><h2>Abrechnung prüfen</h2>'+
      ...
      '<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin:18px 0"><label>Rechnungsart<select data-kind><option value="TR">Teilrechnung</option><option value="SR">Schlussrechnung · 100 %</option></select></label><label>Fertigstellung Fixauftrag %<input data-percent type="number" min="0" max="100" step="0.01"></label><label>Regie jetzt netto €<input data-regie type="number" min="0" step="0.01"></label></div><label>Korrektur / Notiz<textarea data-note rows="2" maxlength="2000" style="width:100%"></textarea></label><div data-result style="margin:16px 0;line-height:1.7"></div><p data-error role="alert" style="color:#a33"></p><button type="button" data-download>Vorschlag herunterladen</button> <a data-open target="_blank" rel="noopener">Als Rechnungsentwurf öffnen</a><p style="font-size:12px;color:#68736a">Der Vorschlag öffnet die Rechnungserstellung am Firmen-PC. Dort kannst du Positionen prüfen und den Entwurf speichern.</p>';
    ...
    dialog.addEventListener('close',()=>dialog.remove());document.body.appendChild(dialog);reset();dialog.showModal();
  }
