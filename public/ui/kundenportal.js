(()=>{
  ...
  function render(){
    ...
    if(selected==="offer"){
      ...
      const accepted=offer.acceptance?.status==="accepted",rows=offer.positions||[],totals=offer.totals||{};
      host.innerHTML=`<section class="card"><p class="eyebrow">Angebot ${esc(offer.number)} · Version ${esc(offer.revision)}</p><h2>${accepted?"Auftrag bestätigt":"Ihr Angebot"}</h2>${offer.intro?`<p>${esc(offer.intro)}</p>`:""}${offer.scopeDescription?`<p><strong>Leistungsumfang:</strong> ${esc(offer.scopeDescription)}</p>`:""}<div class="point-table-wrap"><table class="point-table"><thead><tr><th>Leistung</th><th>Menge</th><th>Einzelpreis</th><th>Gesamt</th></tr></thead><tbody>${rows.map(row=>`<tr><td>${row.groupName?`<span class="meta">${esc(row.groupName)}</span><br>`:""}${esc(row.text)}</td><td>${number(row.quantity)} ${esc(row.unit)}</td><td>${money(row.unitPrice)}</td><td><strong>${money(row.quantity*row.unitPrice)}</strong></td></tr>`).join("")}</tbody><tfoot>${totals.groupDiscount?`<tr><th colspan="3">Rabatt</th><td>− ${money(totals.groupDiscount)}</td></tr>`:""}${totals.globalDiscount?`<tr><th colspan="3">Gesamtrabatt</th><td>− ${money(totals.globalDiscount)}</td></tr>`:""}<tr><th colspan="3">Netto</th><td>${money(totals.net)}</td></tr><tr><th colspan="3">MwSt. ${number(totals.vatRate)} %</th><td>${money(totals.vat)}</td></tr><tr><th colspan="3">Angebotssumme brutto</th><td><strong>${money(totals.gross)}</strong></td></tr></tfoot></table></div>${accepted?`<p class="notice"><strong>Verbindlich beauftragt</strong><br>${esc(new Date(offer.acceptance.acceptedAt).toLocaleString("de-AT"))}${offer.acceptance.customerName?` · ${esc(offer.acceptance.customerName)}`:""}</p>`:`<form id="offerQuestionForm" class="cp-offer-question"><h3>Frage zum Angebot?</h3><p class="meta">Bitte hier schreiben. Farben Krista erhält Ihre Frage sofort als Aufgabe.</p><textarea id="offerQuestion" maxlength="5000" required placeholder="Ihre Frage zum Angebot"></textarea><button type="submit" ${data.preview?"disabled":""}>Frage an Farben Krista senden</button><p id="offerQuestionStatus" role="status"></p></form><label class="cp-offer-confirm"><input id="offerConfirm" type="checkbox"> Ich bestätige, dass ich das Angebot ${esc(offer.number)} verbindlich beauftrage.</label><button id="acceptOffer" class="primary" ${data.preview?"disabled":""}>Angebot verbindlich beauftragen</button><p id="offerAcceptStatus" role="status"></p>`}</section>`;
      ...
      if(!accepted&&!data.preview)$("acceptOffer").onclick=async()=>{const button=$("acceptOffer"),message=$("offerAcceptStatus");if(!$("offerConfirm").checked){message.textContent="Bitte zuerst die verbindliche Beauftragung bestätigen.";return}if(!window.confirm(`Angebot ${offer.number} jetzt verbindlich beauftragen?`))return;button.disabled=true;message.textContent="Beauftragung wird gespeichert …";try{await api("offer/accept",{method:"POST",headers:{"x-csrf-token":data.csrf},body:JSON.stringify({confirmed:true})});await load();$("status").textContent="Vielen Dank. Ihre Beauftragung wurde verbindlich gespeichert und an Farben Krista übermittelt."}catch(error){message.textContent=error.message;button.disabled=false}};
