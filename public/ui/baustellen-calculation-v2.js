...
(function(){
  ...
  function installCss(){
    ...
    const s=document.createElement("style");s.id="kcv2Css";s.textContent=`
      ...
      .kcv2-economy-flow{display:grid;grid-template-columns:repeat(8,minmax(0,1fr));gap:7px}.kcv2-e-step{background:#f8f6f0;border:1px solid #e2ded5;border-radius:11px;padding:10px}.kcv2-e-step small{display:block;color:#707670;font-size:9px}.kcv2-e-step strong{display:block;font-size:14px;margin-top:3px}.kcv2-e-step.main{background:#eef6ef;border-color:#cde0d1}
  ...
  function derive(calc){
    ...
    const material=fixed*pct/100,labor=Math.max(0,fixed-material),rate=num(calc?.billingRate),fixedTarget=rate?labor/rate:0;
    const plannedRegie=rows.reduce((s,r)=>s+((["regie","nachtrag_regie"].includes(r.kind))?num(r.plannedHours):0),0);
    const target=fixedTarget+plannedRegie;
    return {baseNet,added,legacyNachtragRegie,contract,regie,external,other,fixed,pct,material,labor,rate,fixedTarget,target,plannedRegie};
  }
  ...
  function refreshNumbers(){
    ...
    const includedRows=(calculation.positions||[]).filter(row=>row.calcIncluded!==false);set("kcv2PositionSumLabel",includedRows.length===(calculation.positions||[]).length?'Summe Positionen':'Summe Positionen · ohne Alternativen');set("kcv2PositionAmountTotal",money(includedRows.reduce((sum,row)=>sum+num(row.amount),0)));set("kcv2PositionHoursTotal",hours(includedRows.reduce((sum,row)=>sum+(["regie","nachtrag_regie"].includes(row.kind)?num(row.plannedHours):0),0)));
  ...
  function patchEconomy(){
    if(!calculation?.updatedAt||window.BaustellenData.memberIds(currentJob).length>1)return;const host=document.getElementById("bkEconomy");if(!host)return;const d=derive(calculation);const sourceBadge=host.querySelector(".bk-source.missing");if(sourceBadge&&calculation.sourceDocument){sourceBadge.classList.remove("missing");sourceBadge.textContent="PDF / Auftrag zugeordnet"}
    const cards=[...host.querySelectorAll(".bk-card.bk-wide")],card=cards.find(x=>/Vom Auftrag zu den Stunden/i.test(x.textContent||"")),live=window.BaustellenLiveHours?.summary?.(currentJobId),actual=num(live?.total??window.BaustellenData.actualHours(currentJob)),billedNet=num(host.dataset.billedNet),billedGross=num(host.dataset.billedGross),draftNet=num(host.dataset.draftNet),draftGross=num(host.dataset.draftGross),invoiceCount=num(host.dataset.invoiceCount),draftCount=num(host.dataset.draftCount),regieMaterialVk=num(host.dataset.regieMaterialVk),regieMaterialEk=num(host.dataset.regieMaterialEk),regieMaterialProfit=regieMaterialVk-regieMaterialEk,fixedMaterialProfit=d.material*.4,totalMaterialProfit=fixedMaterialProfit+regieMaterialProfit,documentNet=billedNet+draftNet,documentGross=billedGross+draftGross,documentCount=invoiceCount+draftCount,signature=`${calculation.updatedAt}|${actual}|${documentNet}|${documentGross}|${documentCount}|${totalMaterialProfit}`;if(!card||card.dataset.kcv2Patch===signature)return;card.dataset.kcv2Patch=signature;
    const progress=d.target?actual/d.target*100:0,invoiceLabel=documentCount===1?"1 Beleg":`${documentCount} Belege`,draftLabel=draftCount?` · ${draftCount} gespeichert, nicht ausgestellt`:"";
    card.innerHTML=`<div class="bk-section-title"><h3>Vom Auftrag zu den Stunden</h3><span class="bk-source">live aus neuer Kalkulation</span></div><div class="kcv2-economy-flow"><div class="kcv2-e-step"><small>Auftrag inkl. Nachträge</small><strong>${money(d.contract)}</strong></div><div class="kcv2-e-step"><small>Regie / Nachtrag</small><strong>${money(d.regie)}</strong></div><div class="kcv2-e-step"><small>Fremdleistung</small><strong>${money(d.external)}</strong></div><div class="kcv2-e-step"><small>Fahrt / Sonstiges</small><strong>${money(d.other)}</strong></div><div class="kcv2-e-step main"><small>KRISTA fixer Auftrag</small><strong>${money(d.fixed)}</strong></div><div class="kcv2-e-step"><small>Materialertrag gesamt</small><strong>${money(totalMaterialProfit)}</strong><small>fix 40 % ${money(fixedMaterialProfit)} + Regie VK−EK ${money(regieMaterialProfit)}</small></div><div class="kcv2-e-step main"><small>Sollstunden gesamt</small><strong>${hours(d.fixedTarget)} + ${hours(d.plannedRegie)} = ${hours(d.target)}</strong><small>Grundkalkulation + Regie / Nachtrag</small></div><div class="kcv2-e-step main"><small>Gespeicherte / ausgestellte Rechnungen</small><strong>${money(documentNet)} netto</strong><small>${invoiceLabel} · ${money(documentGross)} brutto${draftLabel}</small></div></div><div class="bk-progress"><span style="width:${Math.min(100,Math.max(0,progress))}%;background:${progress>100?'#a84540':'#2f7d4a'}"></span></div><div class="bk-note">${progress.toLocaleString('de-AT',{maximumFractionDigits:1})} % der gesamten kalkulierten Sollstunden verbraucht.</div>`;
  }
