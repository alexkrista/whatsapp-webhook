(() => {
  ...
  function renderPoints(points = []) {
    ...
    host.innerHTML = points.length ? `<p class="cp-help">${points.length} ${points.length === 1 ? "Punkt" : "Punkte"} gespeichert · Neueste zuerst</p>` + points.slice(0, 12).map(row => `<div class="cp-point-row"><strong>${escapeHtml(row.title)}</strong>${escapeHtml(row.area || "Ohne Bereich")} · Zuständig: ${row.responsibility === "bauherr" ? "Bauherr" : "Farben Krista"}${row.photos?.length ? ` · 📷 ${row.photos.length}` : ""}</div>`).join("") : '<p class="cp-help">Noch keine Besprechungspunkte erfasst.</p>';
  }
  ...
  async function savePoint(event) {
    ...
    const jobId = currentJobId, button = document.getElementById("cpPointSave"), files = [...document.getElementById("cpPointPhotos").files];
  ...
  document.getElementById("cpPointPhotos").addEventListener("change", event => { const files = [...event.currentTarget.files]; document.getElementById("cpPointFiles").textContent = files.length ? `${files.length} Foto(s): ${files.map(file => file.name).join(", ")}` : ""; });
  let inviting=false;
