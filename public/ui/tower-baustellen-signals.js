...
(function(){
  const VERSION="20260915-billing-snapshot-1";
  ...
  function renderBillable(jobs){
    ...
    wrap.innerHTML=`<button type="button" id="towerBillable" class="ts-billable"><div><span>Noch abzurechnen</span><small>${esc(billingNote)}</small></div><strong>${billingReady&&!incomplete?money(total):billingPending?'…':'–'}</strong></button><button type="button" class="ts-billable-refresh" data-ts-refresh ${billingPending?'disabled':''}>${billingPending?'Berechne …':'Jetzt neu berechnen'}</button>`;
