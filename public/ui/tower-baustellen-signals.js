...
(function(){
  const VERSION="20260915-billing-snapshot-5";
  ...
  async function apiPut(p,body){const r=await timedFetch(tokenUrl(p),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const t=await r.text();let d;try{d=JSON.parse(t)}catch{}if(!r.ok)throw new Error(d?.error||t||r.statusText);return d}
  ...
  let retryTimer=null;
  let loading=false;
  let lastJobs=[],baustellenHours=null;
  ...
  function target(j){return num(baustellenHours?.byJob?.[String(j?.jobId)]?.target??calc(j).calculatedHours)}
  ...
  function actual(j){return num(baustellenHours?.byJob?.[String(j?.jobId)]?.total)}
  function openHoursFor(j){
    if(closedCollectionMembers.has(String(j?.jobId))||!["Auftrag","Laufend"].includes(j?.status))return 0;
    return num(baustellenHours?.byJob?.[String(j?.jobId)]?.remaining);
  }
  ...
  function billableRows(jobs){
    ...
    for(const j of jobs){
      ...
      if(!performance.hasClosingInvoice&&(performance.amountToInvoice>.005||!performance.complete&&hasBusinessData))rows.push({job:j,hidden:!!j.towerBillingHidden,...performance});
    }
  ...
  async function setBillingHidden(jobId,hidden,button){
    if(button)button.disabled=true;
    try{await apiPut('/admin/api/job/'+encodeURIComponent(jobId)+'/meta',{towerBillingHidden:hidden});await load()}
    catch(error){window.alert('Änderung nicht gespeichert: '+error.message)}
  ...
  function renderBillable(jobs){
    ...
    const B=window.KristaRegieBilling,allRows=billableRows(jobs),rows=allRows.filter(row=>!row.hidden),hiddenRows=allRows.filter(row=>row.hidden),incomplete=rows.filter(row=>!row.complete).length,total=rows.reduce((sum,row)=>sum+num(row.amountToInvoice),0);
    ...
    const rowHtml=(row,hidden)=>`<div class="ts-billable-entry"><a class="ts-billable-link" href="${tokenUrl('/kristine/baustellen')}#${encodeURIComponent(row.job.jobId)}"><span><strong>${esc(row.job.name||row.job.jobId)}</strong><small>#${esc(row.job.jobId)}${row.complete?'':' · prüfen'}</small></span><strong>${row.amountToInvoice!==null?B.formatMoney(row.amountToInvoice):'Daten prüfen'}</strong></a><button type="button" class="ts-billable-hide" data-ts-hidden="${hidden?'0':'1'}" data-job-id="${esc(row.job.jobId)}">${hidden?'Wieder aufnehmen':'Aus Liste nehmen'}</button></div>`;
    dialog.innerHTML=`<div class="ts-billable-head"><div><h2>Noch abzurechnen</h2><small>${esc(snapshotNote())} · Klick öffnet die vollständige Berechnung in der Baustelle.</small></div><div class="ts-billable-actions"><button type="button" data-ts-refresh ${billingPending?'disabled':''}>${billingPending?'Berechne …':'Jetzt neu berechnen'}</button><button type="button" data-ts-close>Schließen</button></div></div><div class="ts-billable-list">${rows.length?rows.map(row=>rowHtml(row,false)).join(''):'<div class="ts-empty">Derzeit ist keine abrechenbare Leistung ohne Schlussrechnung offen.</div>'}${hiddenRows.length?`<details class="ts-hidden"><summary>Ausgeblendete Sonderfälle (${hiddenRows.length})</summary>${hiddenRows.map(row=>rowHtml(row,true)).join('')}</details>`:''}</div>`;
    wrap.querySelector('#towerBillable').onclick=()=>dialog.showModal();document.querySelectorAll('[data-ts-refresh]').forEach(button=>button.onclick=rebuildBilling);dialog.querySelector('[data-ts-close]').onclick=()=>dialog.close();dialog.querySelectorAll('[data-ts-hidden]').forEach(button=>button.onclick=()=>setBillingHidden(button.dataset.jobId,button.dataset.tsHidden==='1',button));dialog.onclick=event=>{if(event.target===dialog)dialog.close()};
  }

  function patchKpis(snapshot,b){
    if(!snapshot)return {totalOpen:null};
    const totalOpen=snapshot.remainingHours;
  ...
  async function load(){if(loading)return;loading=true;try{const [j,b,w]=await Promise.all([api('/admin/api/jobs'),api('/kristine/api/bootstrap').catch(()=>({})),api('/kristool/api/workflows').catch(()=>({workflows:[]}))]);const jobs=j.jobs||[],boot=b||{};lastJobs=jobs;baustellenHours=j.baustellenHours||null;closedCollectionMembers=new Set((j.collections||[]).filter(collection=>String(collection?.status||'')==='Geschlossen').flatMap(collection=>collection?.collectionMemberJobIds||[]).map(String));buildLiveMap(boot);const kpi=patchKpis(j.baustellenHours,boot);render(signals(jobs,boot),w.workflows||[],boot.tasks||[]);if(retryTimer){clearTimeout(retryTimer);retryTimer=null}window.__kristaTowerHours={live:kpi.totalOpen,source:"baustellen",updatedAt:j.baustellenHours?.calculatedAt};billingPending=!billingReady;renderBillable(jobs);try{const snapshot=await brainApi('/tower/billing-snapshot',20000);applyBillingSnapshot(snapshot)}catch(error){if(!billingReady)billingReady=false;console.warn('Tower Abrechnungsstand',error)}finally{billingPending=false;renderBillable(jobs)}}catch(e){console.warn('Tower Baustellen-Signale',e);renderPending('Verbindung wird erneut hergestellt …');billingPending=false;if(!retryTimer)retryTimer=setTimeout(()=>{retryTimer=null;load()},5000)}finally{loading=false}}
