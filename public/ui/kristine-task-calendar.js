...
(function(){
  const VERSION="2026-09-12-appointment-photo-drop-job-workflow-v11";
  ...
  function installStyle(){
    ...
    const s=document.createElement("style");s.id="kristaTaskCalendarCss";s.textContent=`
      ...
      .ktc-photo-box{padding:10px;border:1px solid #d9d5cc;border-radius:12px;background:#faf9f6}.ktc-photo-drop{display:flex;align-items:center;justify-content:center;gap:7px;min-height:78px;padding:13px;border:2px dashed #9eb7a3;border-radius:10px;background:#f0f7f1;color:#245b31;text-align:center;cursor:pointer}.ktc-photo-drop:hover,.ktc-photo-drop.dragover{border-color:#27713d;background:#e3f2e6}.ktc-photo-drop span{font-size:12px;color:#637067}.ktc-photo-list{display:grid;gap:6px;margin-top:8px}.ktc-photo-item{display:flex;align-items:center;gap:8px;padding:7px;border:1px solid #ddd9d0;border-radius:9px;background:#fff;color:#222;text-decoration:none;overflow:hidden}.ktc-photo-item img{width:52px;height:40px;object-fit:cover;border-radius:6px}.ktc-photo-item strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ktc-photo-status{min-height:17px;margin-top:7px;font-size:12px;color:#27713d}
  ...
  function ensureModal(){
    ...
    const drop=bg.querySelector("#ktcPhotoDrop"),input=bg.querySelector("#ktcPhotoInput");drop.addEventListener("click",event=>{if(event.target!==input)input.click()});drop.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();input.click()}});for(const type of ["dragenter","dragover"])drop.addEventListener(type,event=>{event.preventDefault();drop.classList.add("dragover")});for(const type of ["dragleave","drop"])drop.addEventListener(type,event=>{event.preventDefault();drop.classList.remove("dragover")});drop.addEventListener("drop",event=>uploadAppointmentPhotos(event.dataTransfer?.files));input.addEventListener("change",()=>{uploadAppointmentPhotos(input.files);input.value=""});
    return bg;
  }
  ...
  function renderAppointmentPhotos(){
    refreshCurrentTask();const host=document.getElementById("ktcPhotoList");if(!host)return;const files=currentTask?.visitProtocol?.files||[];
    host.innerHTML=files.map(file=>{const href=authenticatedUrl(file.url),image=String(file.mimeType||"").startsWith("image/")?`<img src="${href}" alt="Foto">`:"📎";return `<a class="ktc-photo-item" href="${href}" target="_blank" rel="noopener">${image}<strong>${esc(file.name||"Foto")}</strong></a>`}).join("");
  }

  async function uploadAppointmentPhotos(fileList){
    const files=[...(fileList||[])].filter(file=>String(file.type||"").startsWith("image/"));refreshCurrentTask();if(!currentTask||!files.length)return;const status=document.getElementById("ktcPhotoStatus");status.textContent=`${files.length} Foto${files.length===1?"":"s"} werden gespeichert …`;
    try{const added=[];for(const file of files){const response=await fetch(authenticatedUrl(`/kristine/api/tasks/${encodeURIComponent(currentTask.id)}/visit-file`),{method:"POST",credentials:"same-origin",headers:{"Content-Type":file.type||"application/octet-stream","X-File-Name":encodeURIComponent(file.name||"Foto.jpg")},body:file}),json=await response.json();if(!response.ok)throw new Error(json.error||"Foto konnte nicht gespeichert werden");added.push(json.file)}const taskId=currentTask.id;currentTask.visitProtocol={...(currentTask.visitProtocol||{}),files:[...(currentTask.visitProtocol?.files||[]),...added]};await persistTasks();currentTask=taskById(taskId)||currentTask;renderAppointmentPhotos();status.textContent=`✓ ${added.length} Foto${added.length===1?"":"s"} gespeichert · kommt in die Angebotsmappe mit`;}catch(error){status.textContent="Foto-Übernahme fehlgeschlagen: "+error.message}
  }
  ...
  function open(taskId){
    ...
    document.getElementById("ktcRetry").hidden=true;document.getElementById("ktcPhotoStatus").textContent="Die Fotos bleiben bei der Aufgabe und gehen später in die Angebotsmappe mit.";renderAppointmentPhotos();
