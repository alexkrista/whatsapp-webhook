...
(function(root,factory){
  ...
  function renderCalculation(p){
    ...
    const regie=p.complete&&!p.hasClosingInvoice&&p.regieToInvoice>0&&p.regieInvoiceLinks?.summary&&p.regieInvoiceLinks?.days?`<div class="krb-regie-actions"><div><strong>Regierechnung erstellen</strong><small>Die Zusammenfassung wird bei Bedarf automatisch als eigene PDF-Beilage angeschlossen.</small></div><a href="${escape(p.regieInvoiceLinks.summary)}" target="_blank" rel="noopener">Zusammengefasst + PDF-Beilage</a><a class="primary" href="${escape(p.regieInvoiceLinks.days)}" target="_blank" rel="noopener">Einzeltage übernehmen</a></div>`:"";
    return `<div class="krb-calculation">${summary}<small>${escape(compactCalculation(p))}</small><details><summary>Rechenweg · Fixauftrag und Regie</summary><div class="krb-lines">${calculationLines(p).map(line=>`<div>${escape(line)||"&nbsp;"}</div>`).join("")}</div><button type="button" data-krb-download="${escape(JSON.stringify(p))}">Rechenweg herunterladen (.txt)</button><small>Berechnet: ${escape(new Date(p.calculatedAt).toLocaleString("de-AT"))}${p.dataUpdatedAt?` · Datenstand: ${escape(new Date(p.dataUpdatedAt).toLocaleString("de-AT"))}`:""}</small></details>${renderWarnings(p)}${regie}${actions}</div>`;
  }
