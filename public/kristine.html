<!-- Datei: public/kristine.html · Build 0021.1 -->
<!doctype html>
<html lang="de">
<head>
<meta charset="utf-8">
...
<body>
<header>
  <div><h1>Kristine</h1><div class="sub">Build 0023.2 · Planung & Kartentypen</div></div>
  <a id="adminLink" style="color:white" href="/admin/ui">← Admin</a>
</header>
<main>
<nav>
  <button onclick="showTab('kristool')">🧰 KRISTOOL</button>
...
<section id="control" class="tab">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
    ...
    <div class="report-toolbar">
      <button type="button" onclick="shiftReportDate(-1)" title="Vorheriger Tag">◀</button>
      <button type="button" onclick="openYesterdayReport()">📄 Gestern</button>
      <input id="dailyReportDate" type="date" onchange="openDailyReport(this.value)">
      <button type="button" onclick="shiftReportDate(1)" title="Nächster Tag">▶</button>
    </div>
  </div>
...
function showTab(id){document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.id===id));if(id==='kristool')renderKristool();if(id==='planning')requestAnimationFrame(updatePlanningStickyOffsets)}
...
function renderSystemCards(){
  ...
  const types=Object.keys(CARD_TYPES).filter(type=>type!=='site');
  box.innerHTML=types.map(type=>{const meta=CARD_TYPES[type],showHours=['urlaub','krank','schulung'].includes(type);return `<div class="pool-card type-${esc(type)}" draggable="true" ondragstart="poolDragStart(event,'pooltype:${esc(type)}')" ondragend="poolDragEnd(event)" onclick="selectSystemCard('${type}')"><strong>${meta.icon} ${esc(meta.label)}</strong>${showHours?'<div class="pool-hours">7,8 h</div>':''}</div>`}).join('');
}
...
function planningPoolJobCard(job){
  const number=job.jobId||job.number||'',name=job.name||('#'+number);
  const total=Number(job?.calculation?.calculatedHours||job?.calculatedHours||job?.plannedHours||0);
  const used=consumedHoursForJobUntilYesterday(number);
  const planned=plannedHoursForJobFromToday(number);
  const over=total>0&&(used+planned)>total+0.01;
  const metrics=total>0
    ? `<div class="pool-budget ${over?'over':''}" title="G Gesamt · V verbraucht bis gestern · P geplant ab heute"><span>G ${roundedPlanningHours(total)}</span><span>V ${roundedPlanningHours(used)}</span><span>P ${roundedPlanningHours(planned)}</span></div>`
    : `<div class="pool-budget" title="Noch keine Gesamtstunden kalkuliert"><span>G –</span><span>V ${roundedPlanningHours(used)}</span><span>P ${roundedPlanningHours(planned)}</span></div>`;
  return `<div class="pool-card" draggable="true" ondragstart="poolDragStart(event,'pooljob:${esc(number)}')" ondragend="poolDragEnd(event)" title="In die Planung ziehen"><strong>${esc(number)} · ${esc(name)}</strong>${metrics}</div>`;
}
function renderPlanningPools(){
  renderSystemCards();
  const newJobs=masterJobs.filter(isNewOrderJob).sort((a,b)=>String(a.name||a.jobId).localeCompare(String(b.name||b.jobId),'de'));
  const running=masterJobs.filter(isRunningJob).sort((a,b)=>String(a.name||a.jobId).localeCompare(String(b.name||b.jobId),'de'));
...
function specialPlanHours(a){const type=cardTypeOf(a);if(['urlaub','krank','schulung'].includes(type))return Math.max(0,Number(worktimeRule(a.employeeId,a.date)?.targetHours||fullDayUnproductiveHours(a)));return rawCardHours(a)}
...
async function createSpecialAssignment(type,targetEmployee,date,source=null){
  ...
  const fullDay=['urlaub','krank','schulung'].includes(type),w=modelDayWindow(targetEmployee.id,date),rule=worktimeRule(targetEmployee.id,date);
  ...
  const row={...base,id:id(),cardType:type,jobId:'__'+type+'__',jobName:meta.label,city:'',address:'',employeeId:String(targetEmployee.id),employeeName:targetEmployee.name||String(targetEmployee.id),vehicle:'',from,to,hours:fullDay?Math.max(0,Number(rule?.targetHours||7.8)):rawCardHours({from,to}),note:base.note||meta.label,date};
...
function renderEmployeePlanning(target,title,days){
  ...
  target.innerHTML=`<div class="planning-matrix ${dayMode?'day-matrix':''}">${heads}${rows}</div><div class="perspective-note">Hier ist immer eindeutig, für welchen Mitarbeiter eine Karte gilt. Karte ziehen = Kopie erstellen. Das Original bleibt bestehen; Zielstunden werden aus dem Arbeitsmodell des Zieltags berechnet.</div>`;
}
function renderSitePlanning(target,title,days){
  ...
  target.innerHTML=sortbar+`<div class="planning-matrix ${dayMode?'day-matrix':''}">${heads}${rows}</div><div class="perspective-note">Diese Sicht beantwortet: Wer arbeitet auf welcher Baustelle? Karten werden beim Ziehen kopiert. Das Original bleibt bestehen; Entfernen erfolgt bewusst über ×.</div>`;
}
...
async function addAssignment(){
  ...
  for(const el of selected){
    ...
    else {const fullDay=['urlaub','krank','schulung'].includes(type);if(fullDay)data.assignments.push({...template,date:aDate.value,employeeId:String(employee.id),employeeName:employee.name,from:aFrom.value,to:aTo.value,hours});else {const selected=await askSegmentWindow({employee,date:aDate.value,jobName:meta.label,suggestion:{from:aFrom.value||'08:00',to:aTo.value||'10:00'}});if(!selected)continue;data.assignments.push({...template,date:aDate.value,employeeId:String(employee.id),employeeName:employee.name,from:selected.from,to:selected.to,hours:rawCardHours(selected)})}}
  }
...
function renderDayBar(segments,dayStart,dayEnd){
  ...
  for(const seg of segments){
    ...
    const cls=seg.type==='work'?'seg-work':seg.type==='pause'?'seg-pause':seg.type==='lunch'?'seg-lunch':seg.type==='vacation'?'seg-vacation':seg.type==='sick'?'seg-sick':seg.type==='za'?'seg-za':'seg-holiday';
    const label=`${seg.fromText}–${seg.toText} ${seg.type==='work'?'Arbeit · '+(seg.jobName||seg.jobId||'Baustelle'):seg.type==='pause'?'Pause':seg.type==='lunch'?'Mittag':(seg.jobName||'Abwesenheit')}`;
...
function closeEmployeeActionModal(){document.getElementById('employeeActionBackdrop')?.classList.remove('open');selectedControlEmployeeId='';timeEditorSegments=[];}
...
function segmentTypeLabel(type){return type==='work'?'Arbeit':type==='lunch'?'Mittag':'Pause'}
function renderTimeEditor(){
  const rows=timeEditorSegments.map((seg,index)=>`<tr class="time-editor-row-${esc(seg.type)}"><td><select onchange="updateTimeEditorRow(${index},'type',this.value)"><option value="work" ${seg.type==='work'?'selected':''}>Arbeit</option><option value="lunch" ${seg.type==='lunch'?'selected':''}>Mittag</option><option value="pause" ${seg.type==='pause'?'selected':''}>Pause</option></select></td><td><input type="time" value="${esc(seg.from||'')}" onchange="updateTimeEditorRow(${index},'from',this.value)"></td><td><input type="time" value="${esc(seg.to||'')}" onchange="updateTimeEditorRow(${index},'to',this.value)"></td><td>${seg.type==='work'?`<select onchange="setTimeEditorJob(${index},this)">${jobOptions(seg.jobId)}</select>`:'<span class="small">–</span>'}</td><td><div class="time-editor-actions"><button class="secondary" onclick="moveTimeEditorRow(${index},-1)" title="nach oben">↑</button><button class="secondary" onclick="moveTimeEditorRow(${index},1)" title="nach unten">↓</button><button class="danger" onclick="removeTimeEditorRow(${index})">Löschen</button></div></td></tr>`).join('');
  ...
  const breaks=timeEditorSegments.filter(x=>x.type!=='work').reduce((sum,x)=>{const f=hmToMinutes(x.from),t=hmToMinutes(x.to);return sum+(f!==null&&t!==null&&t>f?t-f:0)},0);
  timeEditorTable.innerHTML=`<table class="time-editor-table"><thead><tr><th>Art</th><th>Von</th><th>Bis</th><th>Baustelle</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="5" class="small">Noch keine Buchung vorhanden.</td></tr>'}</tbody></table><div class="time-editor-summary"><span>Arbeitszeit ${durationText(work)}</span><span>Pause/Mittag ${durationText(breaks)}</span><span>${timeEditorSegments.length} Abschnitt(e)</span></div>`;
}
function updateTimeEditorRow(index,key,value){if(!timeEditorSegments[index])return;timeEditorSegments[index][key]=value;if(key==='type'&&value!=='work'){timeEditorSegments[index].jobId='';timeEditorSegments[index].jobName=''}renderTimeEditor()}
function setTimeEditorJob(index,select){const row=timeEditorSegments[index];if(!row)return;row.jobId=select.value;row.jobName=select.selectedOptions[0]?.dataset.name||''}
function addTimeEditorRow(type='work'){
  const last=timeEditorSegments.at(-1);const from=last?.to||currentViennaHM();
  const firstJob=masterJobs.find(j=>j.active!==false);
  timeEditorSegments.push({id:id(),type,from,to:'',jobId:type==='work'?String(firstJob?.jobId||firstJob?.id||''):'',jobName:type==='work'?String(firstJob?.name||firstJob?.jobName||''):''});renderTimeEditor();
}
...
async function openEmployeeActionModal(employeeId){
  ...
  employeeActionInfo.innerHTML='Lädt Zeitabschnitte …';employeeActionNote.textContent='';employeeActionBackdrop.classList.add('open');
  try{
    const date=chatDate.value||data.today;const result=await api(`/kristine/api/segments/${encodeURIComponent(employeeId)}/${encodeURIComponent(date)}`);
...
async function saveTimeEditor(){
  ...
  const date=chatDate.value||data.today;
  for(const seg of timeEditorSegments){if(!seg.from){employeeActionNote.textContent='Bei jedem Abschnitt muss „Von“ ausgefüllt sein.';return}if(seg.type==='work'&&!seg.jobId){employeeActionNote.textContent='Bitte bei jedem Arbeitsabschnitt eine Baustelle wählen.';return}}
  employeeActionNote.textContent='Speichert …';
  try{
    const result=await api(`/kristine/api/segments/${encodeURIComponent(selectedControlEmployeeId)}/${encodeURIComponent(date)}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({employeeName:employee?.name||'',segments:timeEditorSegments,moveLinked:true})});
    employeeActionNote.textContent=`✓ ${result.segments.length} Abschnitt(e) gespeichert.${result.movedLinkedEntries?` ${result.movedLinkedEntries} Foto-/Material-/Regieeintrag(e) mitverschoben.`:''}`;
    await loadSilent();timeEditorSegments=(result.segments||[]).map(x=>({...x}));renderTimeEditor();
...
function renderKristool(){
  ...
  const today=chatDate?.value||data.today||iso(new Date());
...
function renderControl(){
  ...
  controlList.innerHTML=`<div class="bar-legend"><span><i class="legend-dot" style="background:#2e8b57"></i>Arbeit</span><span><i class="legend-dot" style="background:#c83d3d"></i>Pause</span><span><i class="legend-dot" style="background:#e4a11b"></i>Mittag</span><span><i class="legend-dot" style="background:#dedbd4"></i>offen</span></div><div class="control-grid" style="margin-top:12px">${employees.map(a=>{
    ...
    let segments=employeeDaySegments(a.employeeId,today,s);
    const absencePlan=plans.find(x=>['urlaub','krank','za','feiertag','betriebsurlaub'].includes(cardTypeOf(x)));
    if(absencePlan){const win=modelDayWindow(a.employeeId,today);segments=[{type:cardTypeOf(absencePlan)==='krank'?'sick':cardTypeOf(absencePlan)==='urlaub'?'vacation':cardTypeOf(absencePlan)==='za'?'za':'holiday',from:win.fromMin,to:win.toMin,fromText:win.from,toText:win.to,jobName:cardMeta(absencePlan).label}]}
    ...
    const details=segments.map(x=>`${x.fromText}–${x.toText} ${x.type==='work'?'Arbeit · '+esc(x.jobName||x.jobId||'Baustelle'):x.type==='pause'?'Pause':x.type==='lunch'?'Mittag':esc(x.jobName||'Abwesenheit')}`).join('<br>');
    return `<div class="control-card" style="cursor:pointer" role="button" tabindex="0" onclick="openEmployeeActionModal('${esc(a.employeeId)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openEmployeeActionModal('${esc(a.employeeId)}')}"><div class="control-head"><div><div class="control-name">${esc(a.employeeName)}</div><div class="control-site">📍 ${esc(siteName)}${currentPlan?.address?` <a class="navbtn" target="_blank" onclick="event.stopPropagation()" href="${mapsUrl(currentPlan.address)}">Navi</a>`:''}</div></div><div style="display:flex;align-items:center;gap:8px"><button type="button" class="secondary" style="padding:6px 9px" onclick="event.stopPropagation();openEmployeeActionModal('${esc(a.employeeId)}')">Buchungen bearbeiten</button><span class="status ${esc(s.mode||'idle')}">${esc(statusLabel(s.mode))}</span></div></div><div class="daybar-wrap">${renderAxisLabels(dayStart,dayEnd)}<div class="daybar">${renderDayBar(segments,dayStart,dayEnd)}</div></div><div class="control-meta"><span>⏱ <strong>${durationText(workMinutes)}</strong></span><span>☕ <strong>${durationText(breakMinutes)}</strong></span><span>Letzte Aktion: <strong>${esc(last?.detail||'–')}</strong></span>${regie?`<span>Regie: <strong>${esc(regie.detail)}</strong></span>`:''}</div>${details?`<details class="control-details"><summary>Zeiten ansehen</summary>${details}</details>`:''}</div>`
  }).join('')}</div>`;
}
...
async function addTask(){
  ...
  try{
    ...
    else showNotice('Aufgabe gespeichert. WhatsApp konnte nicht versendet werden.','warn');
  }catch(e){
...
function renderTasks(){
  ...
  taskList.innerHTML=tasks.length?tasks.map(t=>{
    ...
    return `<div class="assignment"><strong>${esc(t.title)}</strong><div class="task-card-meta"><span class="task-badge">${esc(t.taskType||'Aufgabe')}</span><span class="task-badge">${priority}</span></div><span><strong>Für:</strong> ${esc(t.assigneeName||t.assigneeId)} · <strong>Von:</strong> ${esc(t.creatorName||'Chef / Büro')}</span>${t.jobName?`<br><span class="small">🏗 ${esc(t.jobName)}</span>`:''}<br><span class="small">Fällig ${esc(taskDueLabel(t.dueDate))} · ${esc(taskStatusLabel(t))}</span>${t.reminder?`<br><span class="small">ℹ️ ${esc(t.reminder)}</span>`:''}${address?`<br><span class="small">📍 ${esc(address)}</span>`:''}${t.contactEmail?`<br><span class="small">✉ ${esc(t.contactEmail)}</span>`:''}${t.contactPhone?`<br><a class="navbtn" href="tel:${esc(t.contactPhone)}">📞 ${esc(t.contactName||t.contactPhone)}</a>`:''}<div class="actions"><button class="secondary" onclick="openTaskListModal('${t.id}')">Details</button>${t.status!=='done'?`<button class="green" onclick="markTaskDone('${t.id}')">Erledigt</button>`:''}<button class="danger" onclick="removeTask('${t.id}')">×</button></div></div>`
...
function openDailyReport(date){if(!/^\d{4}-\d{2}-\d{2}$/.test(String(date||'')))return;const input=document.getElementById('dailyReportDate');if(input)input.value=date;window.open(url('/admin/daily-report/'+date+'?rebuild=1'),'_blank','noopener')}
function openYesterdayReport(){const d=new Date();d.setDate(d.getDate()-1);openDailyReport(iso(d))}
function shiftReportDate(days){const input=document.getElementById('dailyReportDate');const base=input?.value||data.today||iso(new Date());const d=new Date(base+'T12:00:00');d.setDate(d.getDate()+days);openDailyReport(iso(d))}
