...
class FinanceStore:
    ...
    def status_overrides(self):
        c=self.con()
        try:return {(str(r["source"]),str(r["source_id"])):norm_status(r["status"]) for r in c.execute("SELECT source,source_id,status FROM brain_invoice_status_overrides").fetchall()}
        finally:c.close()
    def set_status_override(self,source,source_id,status=None):
        source=str(source or "").strip();source_id=str(source_id or "").strip()
        if source not in {"WinWorker","KRISTINE"} or not source_id:raise ValueError("Ungültige Rechnung.")
        c=self.con()
        try:
            if status is None:c.execute("DELETE FROM brain_invoice_status_overrides WHERE source=? AND source_id=?",(source,source_id))
            else:c.execute("INSERT INTO brain_invoice_status_overrides(source,source_id,status,updated_at) VALUES(?,?,?,?) ON CONFLICT(source,source_id) DO UPDATE SET status=excluded.status,updated_at=excluded.updated_at",(source,source_id,norm_status(status),datetime.now().isoformat(timespec="seconds")))
            c.commit()
        finally:c.close()
    ...
    def ww(self,include_resolved=False):
        ...
        legacy=self.legacy(); meta=self.meta(); overrides=self.status_overrides(); c=sql("WinWorker_Projekte_Standard")
