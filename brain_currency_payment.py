...
- Zahlungsweg wird bereits bei der Erfassung gewaehlt (SEPA / Revolut / Einzug / Bar).
- Fremdwaehrungen bekommen einen online abrufbaren Kurs zur EUR-Basis.
- Der verwendete Kurs, Stichtag und die Quelle bleiben an der Rechnung gespeichert.
- Fuer spaeteren Karten-/Bankabgleich werden Toleranz und tatsaechlicher Settlement-Kurs
  getrennt vom urspruenglichen Rechnungskurs vorgesehen.
"""
...
def install(ns):
    ...
    def save_fx(area, invoice_id, payload):
        ...
        try:
            ...
            # Im Echtbetrieb gleich an den bestehenden Finance-Metastore spiegeln,
            # damit Revolut/SEPA sofort den richtigen Zahlungsweg sehen.
            if area == "live":
                con.execute("""
    ...
    # Online-Kurs: Frankfurter verwendet EZB-Referenzkurse und kann historische Tage liefern.
    ...
    # Bestehende Analyze-Antwort um erkannte Waehrung ergaenzen.
    original_analyze = app.view_functions.get("incoming_capture_analyze")
    if original_analyze and not getattr(original_analyze, "_krista_currency", False):
        def wrapped_analyze():
            ...
            try:
                ...
                pass
            return response
        wrapped_analyze.__name__ = "incoming_capture_analyze_currency"
        wrapped_analyze._krista_currency = True
        app.view_functions["incoming_capture_analyze"] = wrapped_analyze

    # Neu speichern: FX-/Zahlungsdaten nach dem unveraenderten Kern-Save anhaengen.
    original_save = app.view_functions.get("incoming_capture_save")
    if original_save and not getattr(original_save, "_krista_currency", False):
        ...
        def wrapped_save():
            ...
            try:
                if response.is_json:
                    ...
                    if body.get("ok") and invoice.get("id"):
                        ...
                        invoice.update(save_fx(area, invoice["id"], payload))
            ...
            return response
        wrapped_save.__name__ = "incoming_capture_save_currency"
        wrapped_save._krista_currency = True
        app.view_functions["incoming_capture_save"] = wrapped_save

    # Bearbeitungsdaten um FX-/Zahlungsprofil anreichern.
    edit_data = app.view_functions.get("brain_capture_edit_data")
    if edit_data and not getattr(edit_data, "_krista_currency", False):
        from flask import request
        ...
        def wrapped_edit_data(invoice_id):
            ...
            try:
                ...
                pass
            return response
        wrapped_edit_data.__name__ = "brain_capture_edit_data_currency"
        wrapped_edit_data._krista_currency = True
        app.view_functions["brain_capture_edit_data"] = wrapped_edit_data

    edit_save = app.view_functions.get("brain_capture_edit_save")
    if edit_save and not getattr(edit_save, "_krista_currency", False):
        from flask import request
        ...
        def wrapped_edit_save(invoice_id):
            ...
            return response
        wrapped_edit_save.__name__ = "brain_capture_edit_save_currency"
        wrapped_edit_save._krista_currency = True
        app.view_functions["brain_capture_edit_save"] = wrapped_edit_save

    # Dashboard-Summen immer in EUR, damit USD/CHF nicht einfach mit EUR addiert werden.
    dashboard_view = app.view_functions.get("incoming_capture_dashboard")
    if dashboard_view and not getattr(dashboard_view, "_krista_currency", False):
        from flask import request
        ...
        def wrapped_dashboard():
            ...
            try:
                ...
                try:
                    ensure_schema(con)
                    rows = con.execute("SELECT id,invoice_date,payment_state,currency,gross_amount,fx_rate_to_eur,fx_estimated_eur FROM incoming_invoices").fetchall()
                    def eur_amount(row):
                    ...
                    costs = con.execute("""
                        SELECT a.cost_type,i.currency,i.fx_rate_to_eur,SUM(a.net_amount) net_sum,COUNT(DISTINCT i.id) invoice_count
                        FROM incoming_allocations a JOIN incoming_invoices i ON i.id=a.invoice_id
                        WHERE SUBSTR(i.invoice_date,1,4)=?
                        GROUP BY a.cost_type,i.currency,i.fx_rate_to_eur
                    """, (year,)).fetchall()
                    grouped = {}
                    counts = {}
                    for r in costs:
                        code = _currency(r["currency"]); rate = 1.0 if code == "EUR" else (_float(r["fx_rate_to_eur"]) or 0.0)
                        key = str(r["cost_type"] or "Sonstiges")
                        grouped[key] = grouped.get(key, 0.0) + float(r["net_sum"] or 0) * rate
                        counts[key] = counts.get(key, 0) + int(r["invoice_count"] or 0)
                    dashboard["costSummary"] = [{"costType": k, "netSum": round(v, 2), "invoiceCount": counts.get(k, 0)} for k, v in sorted(grouped.items())]
                finally:
                    con.close()
            ...
            return response
        wrapped_dashboard.__name__ = "incoming_capture_dashboard_currency"
        wrapped_dashboard._krista_currency = True
...
'''
    ...
    script = r'''
<script id="kristaCurrencyPaymentV1">
(function(){
  if(typeof captureCurrency==='undefined'||typeof captureGross==='undefined')return;
  const currencyCodes=%s;
  ...
  fx.innerHTML='<label>Umrechnungsfaktor<div class="capture-fx-rate-line" id="captureFxRateLabel">1 USD =</div><input id="captureFxRate" type="number" step="0.000001" min="0"></label><label>Kurs-Stichtag<input id="captureFxDate" type="date"></label><div><div class="formlabel">EUR-Vergleich</div><div id="captureFxEstimate" class="capture-fx-estimate">–</div><div id="captureFxSource" class="sub"></div></div><button id="captureFxOnline" type="button" class="capture-fx-online">↻ Kurs online</button><label>Abgleich ± %<input id="captureFxTolPercent" type="number" min="0" max="25" step="0.1" value="3"></label><label>oder mindestens ± EUR<input id="captureFxTolEur" type="number" min="0" max="100" step="0.01" value="1.00"></label><div class="capture-fx-hint">Gespeichert wird der verwendete Rechnungskurs. Beim späteren Bank/Revolut-Abgleich darf der tatsächliche Karten-/Bankkurs abweichen; die Toleranz greift nur zusammen mit Lieferant/Referenz/Datum, nie als alleiniger Treffer.</div>';
  ...
  function updateFx(){
    const code=String(captureCurrency.value||'EUR').toUpperCase(),foreign=code!=='EUR';fx.hidden=!foreign;
    if(!foreign){rate.value='1';rateDate.value=captureInvoiceDate?.value||isoToday();rateSource='EUR';source.textContent='';estimate.textContent=money(Number(captureGross.value||0),'EUR');return}
    rateLabel.textContent='1 '+code+' = EUR';if(!rateDate.value)rateDate.value=captureInvoiceDate?.value||isoToday();
    const r=Number(rate.value||0),gross=Number(captureGross.value||0);estimate.textContent=r>0?('≈ '+money(gross*r,'EUR')):'–';source.textContent=rateSource||'Kurs noch nicht gespeichert';
  }
  async function fetchRate(){
    const code=String(captureCurrency.value||'EUR').toUpperCase();if(code==='EUR')return updateFx();online.disabled=true;const old=online.textContent;online.textContent='Lade …';
    try{const day=rateDate.value||captureInvoiceDate?.value||isoToday(),r=await fetch('/incoming/capture/fx-rate?currency='+encodeURIComponent(code)+'&date='+encodeURIComponent(day),{cache:'no-store'}),d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'Kurs nicht verfügbar');rate.value=Number(d.rateToEur||0).toFixed(6);rateDate.value=d.date||day;rateSource=d.source||'Online';updateFx()}catch(e){source.textContent=e.message||String(e)}finally{online.disabled=false;online.textContent=old}}
  function detectFromAnalysis(){
    let a={};try{a=captureAnalysis||{}}catch(_){}
    const code=String(a.currency||'').toUpperCase();if(code&&currencyCodes.includes(code)){captureCurrency.value=code;rate.value=code==='EUR'?'1':'';rateDate.value=captureInvoiceDate?.value||isoToday();rateSource=code==='EUR'?'EUR':'';updateFx();if(code!=='EUR')fetchRate()}
  }

  captureCurrency.addEventListener('change',()=>{const code=String(captureCurrency.value||'EUR').toUpperCase();rate.value=code==='EUR'?'1':'';rateSource=code==='EUR'?'EUR':'';rateDate.value=captureInvoiceDate?.value||isoToday();updateFx()});
  captureGross.addEventListener('input',updateFx);rate.addEventListener('input',()=>{rateSource='manuell';updateFx()});rateDate.addEventListener('change',()=>{rateSource='';updateFx()});online.addEventListener('click',fetchRate);
  if(typeof captureInvoiceDate!=='undefined')captureInvoiceDate.addEventListener('change',()=>{if(!rateDate.value||rateSource==='')rateDate.value=captureInvoiceDate.value;updateFx()});

  if(typeof analyzeCaptureFile==='function'){
    const originalAnalyze=analyzeCaptureFile;analyzeCaptureFile=async function(){const result=await originalAnalyze.apply(this,arguments);detectFromAnalysis();return result};
    if(typeof captureFile!=='undefined')captureFile.onchange=analyzeCaptureFile;
  }
  const originalPayload=typeof capturePayload==='function'?capturePayload:null;
  if(originalPayload){capturePayload=function(){const p=originalPayload.apply(this,arguments),code=String(captureCurrency.value||'EUR').toUpperCase(),r=code==='EUR'?1:Number(rate.value||0);p.paymentMethod=payment.value;p.fxRateToEur=r>0?r:null;p.fxRateDate=rateDate.value||captureInvoiceDate?.value||'';p.fxRateSource=code==='EUR'?'EUR':rateSource;p.fxTolerancePercent=Number(tolP.value||0);p.fxToleranceEur=Number(tolE.value||0);return p}}

  // Beim erneuten Öffnen einer ungeprüften Rechnung FX/Zahlungsweg nachladen.
  ...
  // Fremdwaehrungsbetraege in der Liste nicht als EUR beschriften.
  if(typeof renderCaptureRecent==='function'){
    const originalRecent=renderCaptureRecent;renderCaptureRecent=function(rows){const result=originalRecent.apply(this,arguments),cards=[...captureRecent.querySelectorAll(':scope > .card')];(rows||[]).forEach((x,i)=>{const amount=cards[i]?.querySelector('.invoice-amount');if(!amount)return;amount.textContent=money(x.grossAmount,x.currency||'EUR');if(x.currency&&x.currency!=='EUR'&&x.fxEstimatedEur)amount.textContent+=' · ≈ '+money(x.fxEstimatedEur,'EUR')});return result};
  }

  updateFx();setTimeout(detectFromAnalysis,0);
})();
</script>
''' % json.dumps(list(COMMON_CURRENCIES), ensure_ascii=False)

    page = page.replace("</style>", css + "\n</style>", 1)
    page = page.replace("</body>", script + "\n</body>", 1)
    ns["MOBILE_PAGE"] = page
    print("✅ KRISTINE FX/Zahlungsweg aktiv: Fremdwaehrungen · Online-Kurs · Revolut/SEPA · Kurs gespeichert")
