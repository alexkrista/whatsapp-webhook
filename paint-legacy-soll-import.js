  ...
  app.post("/admin/api/paint/inventory/import-legacy-soll", async (req, res) => {
    ...
    try {
      ...
      for (let r = 9; r < rows.length; r += 1) {
        ...
        const soll = num(row[12]); // Spalte M = Soll; im alten Mischen zugleich Mindestlagerstand.
      ...
      for (const article of Array.isArray(articles) ? articles : []) {
        ...
        matched += 1;
        let touched = false;
        // Das alte Excel kennt nur einen Sollwert. Für den Start setzen wir ihn als
        // Mindest UND Soll. Danach können beide Werte in KRISTINE getrennt angepasst werden.
