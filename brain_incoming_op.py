...
def install(ns):
        ...
        def brain_incoming_open_items():
            try:
                ...
                items = [
                    ...
                    if not str(item.get("docId") or "").strip() in local_doc_ids
                ] + local_items
    ...
    if "kristaIncomingOpenItemsV2" in page:
        ns["MOBILE_PAGE"] = page
        return
...
'''
    ...
    script = r'''
<script id="kristaIncomingOpenItemsV2">
(function(){
  ...
  function rowHtml(r){return `<div class="incoming-op-row"><div class="due">${esc(date(r.dueDate))}</div><div><strong>${esc(r.supplier)}</strong><div class="sub">${esc(r.source||'')} ${r.docId?'· '+esc(r.docId):''}</div></div><div>${esc(r.invoiceNumber||'–')}</div><div>${esc(date(r.invoiceDate))}</div><div class="amount">${esc(money(r.amount,r.currency))}</div><div>${pdfLink(r)}</div></div>`}
  function render(){
  ...
  async function load(){
    ...
    try{const r=await fetch('/incoming/open-items',{cache:'no-store'}),d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'OP-Liste konnte nicht geladen werden');rows=Array.isArray(d.items)?d.items:[];meta.textContent=`${d.count||0} offen · ${money(d.total||0)} · WinWorker + KRISTINE`;render()}catch(e){list.innerHTML=`<div class="empty">${esc(e.message)}</div>`}
  }
...
'''
    ...
    # Alte V1-Ausgabe entfernen, falls brain_incoming_op bereits vorher im
    # Seitenaufbau eingebaut wurde. So gibt es garantiert nur eine OP-Liste.
    import re
    page = re.sub(r'<section id="incomingOpenItemsPanel".*?</section>', '', page, flags=re.S)
    page = re.sub(r'<script id="kristaIncomingOpenItemsV1">.*?</script>', '', page, flags=re.S)
    ...
    ns["MOBILE_PAGE"] = page
    print("✅ Brain OP-Liste V2 aktiv: WinWorker + KRISTINE Echtbetrieb")
