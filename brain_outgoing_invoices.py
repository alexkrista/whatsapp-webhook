...
<div id="addressModal" class="modal hide"><div class="card"><div class="head"><h2>Rechnungsadresse ändern</h2><button data-close="addressModal">×</button></div><p class="small muted">Gilt zuerst für diesen Rechnungslauf. Wenn sich Straße, PLZ oder Ort ändern, kannst du danach auch die Baustellen-Stammdaten aktualisieren.</p><div class="grid"><label style="grid-column:1/-1">Bezeichnung<input id="addressLabel"></label><label>Firma<input id="addressCompany"></label><label>Name<input id="addressName"></label><label>Straße<input id="addressStreet"></label><label>PLZ<input id="addressPostal"></label><label>Ort<input id="addressCity"></label><label>Land<input id="addressCountry"></label><label>Kunden-UID<div class="uid-tools"><input id="addressUid"><button type="button" id="addressUidSearch">Aus WW suchen</button><button type="button" id="addressUidVerify">Prüfen</button></div></label></div><div class="row section"><button id="addressSave" class="primary">Adresse speichern</button><button data-close="addressModal">Abbrechen</button></div></div></div>
...
function renderRun(){const r=selectedRun,draft=(r.invoices||[]).find(x=>x.status==='draft'),nextTr=(r.invoices||[]).filter(x=>x.kind==='TR'&&x.status==='issued').length+1,isSale=r.project_index==null&&!r.project_number,meta=isSale?`Verkauf ohne Projekt · ${esc(r.customer_company||r.customer_name)} · ${esc(r.customer_street)}, ${esc(r.customer_postal_code)} ${esc(r.customer_city)}`:`Projekt ${esc(r.project_number)} · ${esc(r.customer_company||r.customer_name)} · ${esc(r.customer_street)}, ${esc(r.customer_postal_code)} ${esc(r.customer_city)}`;$('workspace').innerHTML=`<div class="card"><div class="head"><div><h2>${esc(r.label)}</h2><div class="muted">${meta}</div></div><div class="row"><button id="editRecipient" class="ghost">Adresse ändern</button><span class="pill ${r.status}">${r.status==='open'?'offen':'abgeschlossen'}</span></div></div><div class="summary"><div>Rechnungsstand<strong>${money(r.currentGross)}</strong></div><div>Zahlungen<strong>${money(r.paidGross)}</strong></div><div>Offen<strong>${money(r.currentOpen)}</strong></div><div>Belege<strong>${r.invoiceCount}</strong></div></div><div class="row">${r.status==='open'&&!r.label.startsWith('WW-Altbestand')?`<button id="newInvoice" class="primary">${draft?'Entwurf bearbeiten':isSale?'Rechnung erstellen':nextTr+'. TR / Rechnung erstellen'}</button>`:''}<button id="newPayment" class="good">Zahlung buchen</button></div></div><div class="card section"><h3>Rechnungen</h3><div id="invoiceList">${invoiceList(r.invoices)}</div><div id="quickView"></div></div><div class="card section"><h3>Zahlungen</h3>${paymentList(r.payments)}</div><div id="editor"></div>`;$('editRecipient').onclick=showRecipient;$('newInvoice')?.addEventListener('click',()=>{editInvoice(draft||null);if(isSale&&!draft){$('invKind').value='RE';$('invKind').dispatchEvent(new Event('change'))}});$('newPayment').onclick=showPayment;wireInvoices()}
function showRecipient(){const r=selectedRun;$('addressLabel').value=r.label||'';$('addressCompany').value=r.customer_company||'';$('addressName').value=r.customer_name||'';$('addressStreet').value=r.customer_street||'';$('addressPostal').value=r.customer_postal_code||'';$('addressCity').value=r.customer_city||'';$('addressCountry').value=r.customer_country||'Österreich';$('addressUid').value=r.customer_uid||'';open('addressModal');$('addressStreet').focus()}
async function saveRecipient(){const before={street:String(selectedRun.customer_street||'').trim(),postalCode:String(selectedRun.customer_postal_code||'').trim(),city:String(selectedRun.customer_city||'').trim()},address={street:$('addressStreet').value.trim(),postalCode:$('addressPostal').value.trim(),city:$('addressCity').value.trim()},addressChanged=before.street!==address.street||before.postalCode!==address.postalCode||before.city!==address.city,projectNumber=String(selectedRun.project_number||'').trim();let label=$('addressLabel').value;for(const [oldValue,newValue] of [[before.street,address.street],[before.postalCode,address.postalCode],[before.city,address.city]])if(oldValue&&newValue&&oldValue!==newValue)label=label.split(oldValue).join(newValue);const d=await api('/api/outgoing/runs/'+selectedRun.id+'/recipient',{method:'PUT',body:JSON.stringify({label,company:$('addressCompany').value,customerName:$('addressName').value,street:address.street,postalCode:address.postalCode,city:address.city,country:$('addressCountry').value,customerUid:$('addressUid').value})});close('addressModal');selectedRun=d.run;let masterUpdated=false,masterError='';if(addressChanged&&/^\d{1,12}$/.test(projectNumber)&&confirm('Adresse wurde geändert. Auch in den Stammdaten dieser Baustelle ändern?')){try{await api('/api/outgoing/projects/'+encodeURIComponent(projectNumber)+'/billing-address',{method:'PUT',body:JSON.stringify(address)});masterUpdated=true}catch(error){masterError=error.message}}const draft=(selectedRun.invoices||[]).find(x=>x.status==='draft');renderRun();if(draft)editInvoice(draft);if(masterError)msg('Rechnungsadresse gespeichert. Stammdaten konnten nicht geändert werden: '+masterError,true);else msg(masterUpdated?'Rechnungsadresse und Baustellen-Stammdaten gespeichert.':'Rechnungsadresse gespeichert. Die Vorschau verwendet jetzt die Korrektur.')}
