...
async function issueInvoice(id){if(!confirm('Rechnung jetzt nummerieren und abschließen?'))return;try{const d=await api('/api/outgoing/invoices/'+id+'/issue',{method:'POST',body:'{}'}),closed=d.projectClosure?.closed?' · Baustelle geschlossen':'';msg('Rechnung '+d.invoice.invoice_number+' erstellt'+closed+'.');await loadRun(selectedRun.id);window.open(d.pdfUrl,'_blank')}catch(e){msg(e.message,true)}}
