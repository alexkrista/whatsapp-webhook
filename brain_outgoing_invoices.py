...
function decorateRegieEditor(){const body=$('lineRows');if(!body)return;let report=0,inMaterial=false;for(const row of [...body.rows]){const cells=[...row.cells],inputs=[...row.querySelectorAll('input')],desc=inputs.find(x=>x.dataset.f==='description'),unitInput=inputs.find(x=>x.dataset.f==='unit'),unit=String(unitInput?.value||'').toUpperCase();row.classList.remove('regie-report-head','regie-section-head','regie-report-sum','regie-material-start');cells.forEach(cell=>cell.classList.remove('regie-hidden'));if(cells[1])cells[1].colSpan=1;if(unit==='TAG'){report++;inMaterial=false;row.classList.add('regie-report-head');const date=(desc?.value||'').match(/\d{1,2}\.\d{1,2}\.\d{4}/)?.[0]||'';if(desc&&date&&!/^\d+\. Bericht/.test(desc.value)){desc.value=`${report}. Bericht - ${date}`;desc.dispatchEvent(new Event('input',{bubbles:true}))}if(cells[0])cells[0].textContent=String(report);if(cells[1])cells[1].colSpan=5;[2,3,4,5].forEach(i=>cells[i]?.classList.add('regie-hidden'));continue}if(unit==='BAUTEIL'||unit==='ARBEIT'||unit==='MATERIAL'){if(unit==='MATERIAL')inMaterial=true;if(desc&&unit!=='MATERIAL'&&desc.value!=='Arbeit'){desc.value='Arbeit';desc.dispatchEvent(new Event('input',{bubbles:true}))}row.classList.add('regie-section-head');if(cells[0])cells[0].textContent='';if(cells[1])cells[1].colSpan=5;[2,3,4,5].forEach(i=>cells[i]?.classList.add('regie-hidden'));continue}if(unit==='SUMME'){row.classList.add('regie-report-sum');if(desc&&desc.value!==`Summe ${report}`){desc.value=`Summe ${report}`;desc.dispatchEvent(new Event('input',{bubbles:true}))}if(cells[0])cells[0].textContent='';if(cells[1])cells[1].colSpan=4;[2,3,4].forEach(i=>cells[i]?.classList.add('regie-hidden'));continue}const labor=['STD','STD.','H','H.'].includes(unit);if(!labor&&!inMaterial){row.classList.add('regie-material-start');inMaterial=true}}}
const editInvoiceBaseStyled=editInvoice;editInvoice=function(x=null){editInvoiceBaseStyled(x);decorateRegieEditor();const body=$('lineRows');if(body)new MutationObserver(()=>decorateRegieEditor()).observe(body,{childList:true})};
