...
async function createLinkedRegieDraft(){const p=linkedRegiePreset(),due=new Date(today()+'T12:00:00');due.setDate(due.getDate()+Number(settings.default_due_days||14));const lines=[];for(const day of p.days){const labor=(day.employees||[]).reduce((s,x)=>s+Number(x.cost||0),0),material=(day.materials||[]).reduce((s,x)=>s+Number(x.cost||0),0),label=new Intl.DateTimeFormat('de-AT').format(new Date(day.date+'T12:00:00'));lines.push({description:`${label}${day.reportNumber?' · Rapport '+day.reportNumber:''}`,quantity:0,unit:'TAG',unitPrice:0,discountPercent:0},{description:`Raum / Bauteil: ${day.component||'Raum / Bauteil'}`,quantity:0,unit:'BAUTEIL',unitPrice:0,discountPercent:0});for(const x of day.employees||[])lines.push({description:x.name||'Mitarbeiter',quantity:Number(x.hours||0),unit:'Std.',unitPrice:Number(x.hours||0)?Number(x.cost||0)/Number(x.hours):0,discountPercent:0});for(const x of day.materials||[])lines.push({description:x.name||'Material',quantity:Number(x.quantity||1),unit:x.unit||'PA',unitPrice:Number(x.unitPrice||0)||(Number(x.cost||0)/Number(x.quantity||1)),discountPercent:0});lines.push({description:`Tagesabschluss · Lohn ${money(labor)} · Material ${money(material)} · Summe ${money(labor+material)}`,quantity:0,unit:'SUMME',unitPrice:0,discountPercent:0})}if(!lines.length){if(p.labor>0)lines.push({description:'Regiearbeit lt. Berichten',quantity:p.hours||1,unit:p.hours?'Std.':'PA',unitPrice:p.hours?p.labor/p.hours:p.labor,discountPercent:0});if(p.material>0)lines.push({description:'Material lt. Berichten',quantity:1,unit:'PA',unitPrice:p.material,discountPercent:20})}const dates=p.days.map(x=>x.date).filter(Boolean).sort(),d=await api('/api/outgoing/invoices',{method:'POST',body:JSON.stringify({runId:selectedRun.id,kind:'TR',issueDate:today(),dueDate:due.toISOString().slice(0,10),serviceFrom:dates[0]||today(),serviceTo:dates[dates.length-1]||today(),taxMode:'AT20',retentionPercent:0,discountPercent:0,cashDiscountPercent:0,subject:'Regiearbeiten · '+selectedRun.label,notes:'Nach Tagen sowie Räumen/Bauteilen gegliedert; alle Werte sind bearbeitbar.',lines})});await loadRun(selectedRun.id);editInvoice(d.invoice)}
