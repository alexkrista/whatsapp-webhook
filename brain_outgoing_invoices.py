...
<div id="saleModal" class="modal hide"><div class="card"><div class="head"><h2>Neue Verkaufsrechnung</h2><button data-close="saleModal">×</button></div><div class="grid"><div style="grid-column:1/-1"><label>Bestehenden Kunden in WinWorker suchen<div class="row"><input id="saleSearch" class="grow" placeholder="Name, Kundennummer, Straße oder Ort"><button type="button" id="saleSearchButton">Suchen</button></div></label><div id="saleResults" class="small"></div></div><label>Firma (falls vorhanden)<input id="saleCustomer" autocomplete="organization"></label><label>Name<input id="saleName" autocomplete="name"></label><label>Straße<input id="saleStreet" autocomplete="street-address"></label><label>PLZ<input id="salePostal" autocomplete="postal-code"></label><label>Ort<input id="saleCity" autocomplete="address-level2"></label><label>Land<input id="saleCountry" value="Österreich" autocomplete="country-name"></label><label style="grid-column:1/-1">Betreff / Verkauf<input id="saleSubject" placeholder="z. B. Materialverkauf"></label></div><div class="row section"><button id="saleSave" class="primary">Rechnung erfassen</button><button data-close="saleModal">Abbrechen</button></div></div></div>
...
function showSaleInvoice(){close('newInvoiceModal');saleSelectedCustomer=null;for(const id of ['saleSearch','saleCustomer','saleName','saleStreet','salePostal','saleCity','saleSubject'])$(id).value='';$('saleResults').innerHTML='<span class="muted">Bestehenden WW-Kunden auswählen oder die Adresse darunter neu eingeben.</span>';$('saleCountry').value='Österreich';open('saleModal');$('saleSearch').focus()}
function selectSaleCustomer(c){saleSelectedCustomer=c;$('saleCustomer').value=c.company||'';$('saleName').value=c.name||'';$('saleStreet').value=c.street||'';$('salePostal').value=c.postalCode||'';$('saleCity').value=c.city||'';$('saleCountry').value=c.country||'Österreich';$('saleResults').innerHTML=`<div class="project active"><strong>✓ ${esc(c.company||c.name)}</strong><div class="small muted">${esc(c.address||[c.street,[c.postalCode,c.city].filter(Boolean).join(' ')].filter(Boolean).join(', '))}${c.customerNumber?' · KdNr '+esc(c.customerNumber):''}</div></div>`}
