...
async function saveRun(){const p=selectedProject,d=await api('/api/outgoing/runs',{method:'POST',body:JSON.stringify({projectIndex:p.projectIndex,projectNumber:p.projectNumber,customerIndex:p.customerIndex,projectTitle:p.title,label:$('runLabel').value,customerUid:$('runUid').value,company:$('runCompany').value,customerName:$('runName').value,street:$('runStreet').value,postalCode:$('runPostal').value,city:$('runCity').value,country:$('runCountry').value})});close('runModal');await loadRuns();await loadRun(d.run.id);if(linkedProgressProposal())await openProgressProposal()}
...
async function loadRun(id){const d=await api('/api/outgoing/runs/'+id);selectedRun=d.run;if(selectedRun.project_index==null){selectedProject=null;document.querySelectorAll('.project').forEach(x=>x.classList.remove('active'));$('runs').innerHTML='<div class="muted">Einzelverkauf ohne Baustelle.</div>'}editing=editing?(selectedRun.invoices||[]).find(x=>x.id===editing.id)||null:null;$('empty').classList.add('hide');$('workspace').classList.remove('hide');renderRun();await loadRuns()}
