...
def install(ns):
    ...
    def outgoing_line_catalog():
        ...
        if not callable(kristine_api_request):
            return jsonify({"ok": True, "employees": [], "materials": []})
        try:
...
function enhanceRegieEditor(){decorateRegieEditor();addEditorInsertButtons();addReportDiscountControls();mountPricingFactbox();mountInvoiceLiveTotals();wireEditorCatalogs()}
