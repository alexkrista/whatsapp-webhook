...
function wireEditorCatalogs(){if(!editing||editing.status!=='draft')return;ensureLineCatalog().then(catalog=>{catalogDatalist('outgoingEmployeeChoices',catalog.employees);catalogDatalist('outgoingMaterialChoices',catalog.materials);let inMaterial=false;for(const row of [...($('lineRows')?.rows||[])]){const inputs=[...row.querySelectorAll('input')],desc=inputs.find(x=>x.dataset.f==='description'),unit=String(inputs.find(x=>x.dataset.f==='unit')?.value||'').trim().toUpperCase(),price=inputs.find(x=>x.dataset.f==='unitPrice');if(unit==='TAG'){inMaterial=false;continue}if(unit==='MATERIAL'){inMaterial=true;continue}if(['BAUTEIL','ARBEIT','SUMME'].includes(unit)){if(unit==='SUMME')inMaterial=false;continue}const labor=['STD','STD.','H','H.'].includes(unit);if(!labor&&!inMaterial)inMaterial=true;const items=labor?catalog.employees:(inMaterial?catalog.materials:[]),listId=labor?'outgoingEmployeeChoices':(inMaterial?'outgoingMaterialChoices':'');if(!desc||!listId)continue;desc.setAttribute('list',listId);desc.setAttribute('autocomplete','off');if(!desc.dataset.catalogWired){desc.dataset.catalogWired='1';desc.addEventListener('change',()=>applyCatalogChoice(row,items))}if(labor||(inMaterial&&Number(price?.value||0)<=0))applyCatalogChoice(row,items)}})}
