...
async function saveRecipient(){let label=$('addressLabel').value;for(const [before,after] of [[selectedRun.customer_street,$('addressStreet').value],[selectedRun.customer_postal_code,$('addressPostal').value],[selectedRun.customer_city,$('addressCity').value]])if(before&&after&&before!==after)label=label.split(before).join(after);const d=await api('/api/outgoing/runs/'+selectedRun.id+'/recipient',{method:'PUT',body:JSON.stringify({label,company:$('addressCompany').value,customerName:$('addressName').value,street:$('addressStreet').value,postalCode:$('addressPostal').value,city:$('addressCity').value,country:$('addressCountry').value,customerUid:$('addressUid').value})});close('addressModal');selectedRun=d.run;const draft=(selectedRun.invoices||[]).find(x=>x.status==='draft');renderRun();if(draft)editInvoice(draft);msg('Rechnungsadresse gespeichert. Die Vorschau verwendet jetzt die Korrektur.')}
