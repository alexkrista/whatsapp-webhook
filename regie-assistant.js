...
function registerRegieAssistant(app, options) {
  ...
  function normalizeEmployee(row) {
    ...
    return {
      id: clean(row?.id || row?.employeeId, 100),
  ...
  function calculateTotals(report) {
    ...
    const laborTotal = round(laborHours * num(report.hourlyRate));
  ...
  async function nextReportNumber(jobId, reports) {
    const prefix = safeId(jobId);
    const serials = (reports || [])
      .filter(row => String(row.jobId) === String(jobId))
      .map(row => Number(String(row.reportNumber || "").match(/(\d{3})$/)?.[1] || 0));
    return `${prefix}${String((serials.length ? Math.max(...serials) : 0) + 1).padStart(3, "0")}`;
  }
  ...
  async function storeInJobFile(report) {
    ...
    const item = {
      ...
      employeeDetails: report.employees.map(row => ({ name: row.name, hours: row.hours, cost: round(num(row.hours) * num(report.hourlyRate)) })),
  ...
  async function persistReport(body, finish) {
    ...
    const report = {
      ...
      reportNumber: clean(body.reportNumber || existing.reportNumber, 60) || await nextReportNumber(jobId, reports),
  ...
  function printHtml(report, meta) {
    const employeeRows = report.employees.map(row => `<tr><td>${esc(row.name)}</td><td>${esc(row.from || "")}</td><td>${esc(row.to || "")}</td><td class="n">${num(row.hours).toLocaleString("de-AT")} Std</td><td class="n">${money(num(row.hours) * report.hourlyRate)}</td></tr>`).join("");
    const materialRows = report.materials.map(row => `<tr><td>${esc(row.product)}</td><td>${esc(row.supplier)}</td><td class="n">${num(row.quantity).toLocaleString("de-AT")} ${esc(row.unit)}</td><td class="n">${money(row.salePrice)}</td><td class="n">${money(num(row.quantity) * num(row.salePrice))}</td></tr>`).join("");
...
@page{size:A4;margin:18mm 18mm 20mm}*{box-sizing:border-box}body{font-family:Titillium,Arial,sans-serif;color:#142019;font-size:12px;margin:0}.page{min-height:257mm;display:flex;flex-direction:column}.head{display:flex;justify-content:space-between;align-items:flex-start}.logo{width:46mm;height:auto}.address{line-height:1.35;margin-top:15mm}.project{text-align:right;margin-top:9mm}.project strong{font-size:13px}.title{margin-top:14mm;border-bottom:2px solid #31583b;padding-bottom:5px;display:flex;align-items:end;justify-content:space-between}.title h1{font-size:23px;margin:0}.meta{display:grid;grid-template-columns:115px 1fr;gap:4px 10px;margin:12px 0 18px}.description{white-space:pre-wrap;line-height:1.45}.section{margin-top:13px;font-size:15px;font-weight:600;color:#31583b}table{width:100%;border-collapse:collapse;margin-top:6px}th{font-weight:600;text-align:left;border-bottom:1.5px solid #31583b;padding:5px 6px}td{padding:6px;border-bottom:1px solid #d9ded9}.n{text-align:right;white-space:nowrap}.totals{margin:18px 0 0 auto;width:75mm}.totals div{display:flex;justify-content:space-between;padding:4px 2px}.totals .net{border-top:2px solid #31583b;font-weight:600}.totals .gross{border-top:1.5px solid #31583b;font-weight:600;font-size:13px}.accept{margin-top:22px;line-height:1.45}.signatures{display:grid;grid-template-columns:1fr 1fr;gap:22mm;margin-top:22mm}.signature{border-top:1px solid #333;padding-top:4px}.footer{margin-top:auto;border-top:1px solid #31583b;padding-top:6px;font-size:9px;color:#49604f;display:flex;justify-content:space-between}@media print{.no-print{display:none!important}}
</style></head><body><main class="page"><header class="head"><div class="address">${address.map(esc).join("<br>")}</div><div><img class="logo" src="/public/krista-logo.png" alt="Krista"><div class="project"><strong>Projekt ${esc(report.jobId)}</strong><br>${esc(report.jobName)}</div></div></header><section class="title"><h1>Regiebericht</h1><span>${dateLabel(report.date)}</span></section><div class="meta"><strong>Rapport Nr.</strong><span>${esc(report.reportNumber)}</span><strong>Baustelle</strong><span>${esc(report.jobName)}</span><strong>Beschreibung</strong><span class="description">${esc(report.description)}</span></div><div class="section">Arbeitszeit</div><table><thead><tr><th>Mitarbeiter</th><th>Von</th><th>Bis</th><th class="n">Stunden</th><th class="n">Betrag</th></tr></thead><tbody>${employeeRows}</tbody></table>${materialRows ? `<div class="section">Material</div><table><thead><tr><th>Material</th><th>Lieferant</th><th class="n">Menge</th><th class="n">Einzelpreis</th><th class="n">Betrag</th></tr></thead><tbody>${materialRows}</tbody></table>` : ""}<div class="totals"><div><span>Arbeit</span><strong>${money(report.totals.laborTotal)}</strong></div><div><span>Material</span><strong>${money(report.totals.materialTotal)}</strong></div><div class="net"><span>Netto</span><strong>${money(report.totals.net)}</strong></div><div><span>20 % MwSt.</span><strong>${money(report.totals.vat)}</strong></div><div class="gross"><span>Brutto</span><strong>${money(report.totals.gross)}</strong></div></div><div class="accept">Die angeführten Arbeiten und Materialien wurden ordnungsgemäß ausgeführt bzw. geliefert. Mit der Unterschrift bestätigt der Auftraggeber die Richtigkeit dieses Regieberichts.</div><div class="signatures"><div class="signature">Ort, Datum / Auftraggeber</div><div class="signature">Unterschrift Auftraggeber</div></div><footer class="footer"><span>Krista GmbH · Studa 104 · 6800 Feldkirch</span><span>Regiebericht ${esc(report.reportNumber)}</span></footer></main><script>if(new URLSearchParams(location.search).has('print'))setTimeout(()=>print(),250)<\/script></body></html>`;
  }
  ...
  app.get("/kristine/api/regie-reports/next-number", async (req, res) => {
    ...
    res.json({ ok: true, reportNumber: await nextReportNumber(jobId, reports) });
  });
  app.get("/kristine/api/regie-reports/time-suggestions", async (req, res) => {
    ...
    const [events, assignments] = await Promise.all([readJson(TIME_EVENTS, []), readJson(ASSIGNMENTS, [])]);
    const assigned = assignments.filter(row => String(row.jobId) === jobId && String(row.date) === date).map(row => normalizeEmployee({ id: row.employeeId, name: row.employeeName, from: row.from || "07:00", to: row.to || "", hours: row.hours || row.durationHours }));
    ...
    const eventRows = ids.flatMap(id => buildSegments(events, id, date)).filter(row => String(row.jobId) === jobId).map(normalizeEmployee);
    const suggestions = [...new Map([...eventRows, ...assigned].filter(row => row.hours > 0).map(row => [`${row.id}-${row.from}-${row.to}`, row])).values()];
