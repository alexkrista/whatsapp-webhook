...
(function(){
  ...
  function init(){installTools();const pay=document.getElementById('rows'),dd=document.getElementById('directDebitRows'),range=document.getElementById('directDebitRange');if(pay)new MutationObserver(()=>{if(!busy&&Date.now()>=muteUntil)scheduleRefresh()}).observe(pay,{childList:true,subtree:true});if(dd)new MutationObserver(()=>{if(!busy&&Date.now()>=muteUntil)scheduleRefresh()}).observe(dd,{childList:true,subtree:true});if(range)range.addEventListener('change',renderDebits);setTimeout(refresh,180)}
