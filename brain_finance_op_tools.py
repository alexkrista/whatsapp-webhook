...
(function(){
  ...
  const rowAmount=x=>x?.paymentMethod==='direct_debit'?Number(x.amount||0):Number(x?.paymentAmount??x?.amount??0);
  let data={items:[],directDebit:[],unclassified:[]},payNodes=[],busy=false,refreshTimer=0,muteUntil=0;

  function total(rows){const by={};(rows||[]).forEach(x=>{const c=x.currency||'EUR';by[c]=(by[c]||0)+rowAmount(x)});return Object.entries(by).map(([c,n])=>money(n,c)).join(' · ')||money(0)}
  function sortRows(rows,mode){rows=[...(rows||[])];const due=x=>String(x.dueDate||x.expectedDebitDate||x.invoiceDate||'9999-12-31'),supplier=x=>String(x.supplier||'').toLocaleLowerCase('de');if(mode==='amount_desc')rows.sort((a,b)=>rowAmount(b)-rowAmount(a)||due(a).localeCompare(due(b)));else if(mode==='amount_asc')rows.sort((a,b)=>rowAmount(a)-rowAmount(b)||due(a).localeCompare(due(b)));else if(mode==='supplier')rows.sort((a,b)=>supplier(a).localeCompare(supplier(b),'de')||due(a).localeCompare(due(b)));else rows.sort((a,b)=>due(a).localeCompare(due(b))||supplier(a).localeCompare(supplier(b),'de')||rowAmount(a)-rowAmount(b));return rows}
  function filteredDebits(){const horizon=Number(document.getElementById('directDebitRange')?.value||31);return (data.directDebit||[]).filter(x=>{const d=daysTo(x.expectedDebitDate||x.dueDate);return d==null||d<=horizon})}
  ...
  function renderDebits(){const box=document.getElementById('directDebitRows'),meta=document.getElementById('directDebitMeta');if(!box||!meta)return;muteUntil=Math.max(muteUntil,Date.now()+400);const rows=sortRows(filteredDebits(),'due'),range=Number(document.getElementById('directDebitRange')?.value||31);meta.innerHTML=`<strong>${rows.length} erwartete Einzüge</strong>${rows.length?' · '+esc(total(rows)):''} · nächste ${range===31?'1 Monat':'7 Tage'} und alle überfälligen`;if(!rows.length){box.innerHTML='<div class="empty">Keine erwarteten Einzüge in diesem Zeitraum.</div>';return}const groups=new Map();rows.forEach(x=>{const k=String(x.supplier||'Ohne Lieferant');if(!groups.has(k))groups.set(k,[]);groups.get(k).push(x)});box.innerHTML=[...groups.entries()].map(([supplier,items])=>`<div class="dd-group"><div class="dd-head"><strong>${esc(supplier)}</strong><span>${items.length} Rechnung(en) · ${esc(total(items))}</span></div>${items.map(x=>`<div class="dd-row"><div><strong>${esc(date(x.expectedDebitDate||x.dueDate))}</strong><div class="sub">erwarteter Einzug</div></div><div><div>${esc(x.invoiceNumber||'–')}</div><div class="sub">${esc(x.source||'')}</div></div><div class="amount">${esc(money(rowAmount(x),x.currency))}</div><div>${debitStatus(x)}</div><div class="sub">${x.wwStatusRaw?'WW: '+esc(x.wwStatusRaw):''}</div><div class="dd-actions">${x.path?`<a class="pdf" href="/pdf?path=${encodeURIComponent(x.path)}" target="_blank">PDF</a>`:'–'}<button type="button" class="dd-normal" data-dd-normal="${esc(x.source)}|${esc(x.id)}">Auf Überweisung</button></div></div>`).join('')}</div>`).join('')}

  async function changeDebitToTransfer(button){const [source,id]=String(button.dataset.ddNormal||'').split('|');if(!source||!id)return;if(!confirm('Zahlungsart auf normale Überweisung ändern?\n\nDie Rechnung bleibt offen und wechselt zu „Zu zahlende Rechnungen“.'))return;button.disabled=true;try{const response=await fetch('/incoming/payment-meta',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source,id,paymentMethod:'transfer',note:'Zahlungsart manuell auf normale Überweisung geändert'})}),result=await response.json();if(!response.ok||!result.ok)throw Error(result.error||'Zahlungsart konnte nicht geändert werden');location.reload()}catch(error){alert(error.message||error);button.disabled=false}}
  ...
  function init(){installTools();const pay=document.getElementById('rows'),dd=document.getElementById('directDebitRows'),range=document.getElementById('directDebitRange');if(pay)new MutationObserver(()=>{if(!busy&&Date.now()>=muteUntil)scheduleRefresh()}).observe(pay,{childList:true,subtree:true});if(dd){new MutationObserver(()=>{if(!busy&&Date.now()>=muteUntil)scheduleRefresh()}).observe(dd,{childList:true,subtree:true});dd.addEventListener('click',event=>{const button=event.target.closest('[data-dd-normal]');if(button)changeDebitToTransfer(button)})}if(range)range.addEventListener('change',renderDebits);setTimeout(refresh,180)}
