...
(function(){
  ...
  let data={items:[],directDebit:[],unclassified:[]},payNodes=[],busy=false,refreshTimer=0;
  ...
  function init(){installTools();const pay=document.getElementById('rows'),dd=document.getElementById('directDebitRows');if(pay)new MutationObserver(()=>{if(!busy)scheduleRefresh()}).observe(pay,{childList:true,subtree:true});if(dd)new MutationObserver(()=>{if(!busy)scheduleRefresh()}).observe(dd,{childList:true,subtree:true});setTimeout(refresh,180)}
