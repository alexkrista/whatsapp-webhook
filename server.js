...
app.post("/admin/api/job/:jobId/offer-pdf/render",express.text({type:"text/html",limit:"5mb"}),async(req,res)=>{if(!requireAdmin(req,res))return;try{
  ...
  const pdf=await renderOfferHtmlPdf(req.body),storedName=`angebot-${number}-v${revision}.pdf`,dir=documentationDir(jobId),file=path.join(dir,storedName),immutableDir=path.join(DATA_DIR,jobId,"_offers"),immutableFile=path.join(immutableDir,storedName);await ensureDir(dir);await ensureDir(immutableDir);const tmp=file+"."+crypto.randomUUID()+".tmp";try{await fsp.writeFile(tmp,pdf);await fsp.rename(tmp,file);await fsp.copyFile(file,immutableFile)}finally{await fsp.rm(tmp,{force:true}).catch(()=>{})}
