...
async function readVisitWorkflows(){let rows=[];try{rows=JSON.parse(await fsp.readFile(visitWorkflowFile,"utf8"))}catch{}if(!rows.length){try{const tasks=JSON.parse(await fsp.readFile(path.join(DATA_DIR,"_kristine","tasks.json"),"utf8")),task=tasks.find(x=>x.id==="x17lngkncodmtirmgjh");if(task){const now=new Date().toISOString();rows=[{id:`workflow-${Date.now()}-initial`,key:`${task.id}:order`,taskId:task.id,target:"order",status:"order_ready_for_approval",title:task.title||"Fenster malen",customer:task.contactName||"Oskar Latzer",address:task.address||"Im Tobel 14, 6820 Frastanz",appointment:task.appointment||null,protocol:task.visitProtocol||{},createdAt:now,updatedAt:now,timeline:[{type:"call",label:"Anruf / Anfrage",at:task.createdAt||now},{type:"appointment",label:"Termin",at:now},{type:"protocol",label:"Vor-Ort-Protokoll",at:now},{type:"order",label:"Auftragsmappe vorbereitet",at:now}]}];await writeVisitWorkflows(rows)}}catch{}}return rows}
