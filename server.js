...
app.put("/admin/api/job/:jobId/offer-draft", async (req, res) => {
  ...
  try{
    ...
    const measurement={mode:body.measurement?.mode==="estimate"?"estimate":"exact"};for(const key of ["floorArea","wallArea","ceilingArea","facadeArea","woodArea","undersideArea","perimeter","doors","windows","radiators","hours","materialValue","months"])measurement[key]=Math.max(0,Math.min(1000000,Number(body.measurement?.[key]||0)));
