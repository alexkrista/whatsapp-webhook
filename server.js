...
app.put("/admin/api/job/:jobId/offer-draft", async (req, res) => {
  ...
  try{
    ...
    const existingDraft=await fsp.readFile(offerDraftPath(jobId),"utf8").then(JSON.parse).catch(()=>null);draft.offerNumber=/^\d{7}$/.test(String(existingDraft?.offerNumber||""))?String(existingDraft.offerNumber):"";draft.offerCreatedAt=draft.offerNumber?String(existingDraft?.offerCreatedAt||draft.updatedAt):"";draft.offerRevision=draft.offerNumber?Math.max(1,Number(existingDraft?.offerRevision||1)+1):0;
