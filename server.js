...
app.put("/admin/api/job/:jobId/offer-draft", async (req, res) => {
  ...
  try{
    ...
    const measurement={mode:body.measurement?.mode==="estimate"?"estimate":"exact"};for(const key of ["floorArea","wallArea","ceilingArea","roomLength","roomWidth","roomHeight","openingsDeduction","facadeArea","woodArea","undersideArea","perimeter","doors","windows","radiators","hours","months","hourlyRate"])measurement[key]=Math.max(0,Math.min(1000000,Number(body.measurement?.[key]||(key==="hourlyRate"?75:0))));measurement.materialMarkupPct=Math.max(-100,Math.min(10000,Number(body.measurement?.materialMarkupPct??body.measurement?.materialPercent??15)));
    ...
    draft.version=4;draft.scopeAuto=body.scopeAuto!==false;draft.rooms=(Array.isArray(body.rooms)?body.rooms:[]).slice(0,50).map((r,i)=>({id:String(r?.id||`room-${i+1}`).slice(0,80),name:String(r?.name||`Raum ${i+1}`).trim().slice(0,160),mode:r?.mode==="estimate"?"estimate":"exact",floorArea:Math.max(0,Math.min(10000,Number(r?.floorArea||0))),length:Math.max(0,Math.min(1000,Number(r?.length||0))),width:Math.max(0,Math.min(1000,Number(r?.width||0))),height:Math.max(0,Math.min(100,Number(r?.height||0))),openingsDeduction:Math.max(0,Math.min(10000,Number(r?.openingsDeduction||0))),includeWalls:r?.includeWalls!==false,includeCeiling:r?.includeCeiling!==false}));
