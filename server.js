...
const { renderOfferHtmlPdf, appendOfferLegalAnnex } = require("./offer-html-pdf");
...
app.post("/admin/api/job/:jobId/offer-pdf/render",express.text({type:"text/html",limit:"5mb"}),async(req,res)=>{if(!requireAdmin(req,res))return;try{
  ...
  const pdf=await renderOfferHtmlPdf(req.body).then(appendOfferLegalAnnex),storedName=`angebot-${number}-v${revision}.pdf`,dir=documentationDir(jobId),file=path.join(dir,storedName),immutableDir=path.join(DATA_DIR,jobId,"_offers"),immutableFile=path.join(immutableDir,storedName);await ensureDir(dir);await ensureDir(immutableDir);const tmp=file+"."+crypto.randomUUID()+".tmp";try{await fsp.writeFile(tmp,pdf);await fsp.rename(tmp,file);await fsp.copyFile(file,immutableFile)}finally{await fsp.rm(tmp,{force:true}).catch(()=>{})}
  const rows=await readDocumentation(jobId),id=`offer-${crypto.createHash("sha256").update(`${jobId}|${number}|${revision}`).digest("hex").slice(0,20)}`,item={id,type:"offer",name:`Angebot ${number}.pdf`,offerNumber:number,offerRevision:revision,customerVisible:true,storedName,source:"offer-browser-render",legalAnnexVersion:OFFER_TERMS.version,renderedAt:new Date().toISOString(),importedAt:new Date().toISOString(),url:`/admin/api/job/${encodeURIComponent(jobId)}/documentation/file?name=${encodeURIComponent(storedName)}`},next=rows.filter(row=>row?.id!==id&&row?.storedName!==storedName);next.unshift(item);await writeDocumentation(jobId,next.slice(0,1000));await appendJobHistory(jobId,{type:"offer_pdf_stored",title:`Angebot ${number} als verbindliche PDF gespeichert`,detail:`Version ${revision} · identisch für Druck, Versand, Dokumente und Kundenportal; AGB und Rücktrittsunterlagen angehängt`,source:"KRISTINE Angebot",data:{number,revision,storedName,legalAnnexVersion:OFFER_TERMS.version}}).catch(()=>{});res.json({ok:true,item,pdfUrl:item.url});
...
app.post("/admin/api/job/:jobId/offer-pdf/upload",express.raw({type:["application/pdf","application/octet-stream"],limit:"25mb"}),async(req,res)=>{if(!requireAdmin(req,res))return;try{
  ...
  const pdf=await appendOfferLegalAnnex(originalPdf),storedName=`angebot-${number}-v${revision}.pdf`,dir=documentationDir(jobId),file=path.join(dir,storedName),immutableDir=path.join(DATA_DIR,jobId,"_offers"),immutableFile=path.join(immutableDir,storedName),source=accepted?"offer-approved-correction":"offer-approved-original",approvedAt=new Date().toISOString();await ensureDir(dir);await ensureDir(immutableDir);if(accepted){const historyDir=path.join(immutableDir,"history"),stamp=approvedAt.replace(/[:.]/g,"-");await ensureDir(historyDir);if(await fsp.stat(immutableFile).then(()=>true).catch(()=>false))await fsp.copyFile(immutableFile,path.join(historyDir,`angebot-${number}-v${revision}-vor-korrektur-${stamp}.pdf`));}const tmp=file+"."+crypto.randomUUID()+".tmp";try{await fsp.writeFile(tmp,pdf);await fsp.rename(tmp,file);await fsp.copyFile(file,immutableFile);await fsp.writeFile(immutableFile+".json",JSON.stringify({offerNumber:number,offerRevision:revision,storedName,source,approvedAt,sha256:crypto.createHash("sha256").update(pdf).digest("hex")},null,2),"utf8")}finally{await fsp.rm(tmp,{force:true}).catch(()=>{})}
  const rows=await readDocumentation(jobId),id=`offer-${crypto.createHash("sha256").update(`${jobId}|${number}|${revision}`).digest("hex").slice(0,20)}`,item={id,type:"offer",name:`Angebot ${number}.pdf`,offerNumber:number,offerRevision:revision,customerVisible:true,storedName,source,legalAnnexVersion:OFFER_TERMS.version,approvedAt,importedAt:approvedAt,url:`/admin/api/job/${encodeURIComponent(jobId)}/documentation/file?name=${encodeURIComponent(storedName)}`},next=rows.filter(row=>row?.id!==id&&row?.storedName!==storedName);next.unshift(item);await writeDocumentation(jobId,next.slice(0,1000));await appendJobHistory(jobId,{type:accepted?"offer_pdf_corrected_after_acceptance":"offer_pdf_approved",title:accepted?`Freigegebene PDF zu Angebot ${number} korrigiert`:`Freigegebene Original-PDF für Angebot ${number} übernommen`,detail:`Version ${revision} · Originalseiten unverändert, AGB und Rücktrittsunterlagen angehängt${accepted?" · Kundenauftrag und Annahmezeitpunkt unverändert":""}`,source:"KRISTINE Angebot",data:{number,revision,storedName,originalBytes:originalPdf.length,bytes:pdf.length,legalAnnexVersion:OFFER_TERMS.version,customerAcceptanceUnchanged:accepted}}).catch(()=>{});res.json({ok:true,item,pdfUrl:item.url,correctedAfterAcceptance:accepted});
