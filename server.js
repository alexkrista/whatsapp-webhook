...
app.post("/admin/api/job/:jobId/offer-pdf/render",express.text({type:"text/html",limit:"5mb"}),async(req,res)=>{if(!requireAdmin(req,res))return;try{
  ...
  const pdf=await renderOfferHtmlPdf(req.body).then(appendOfferLegalAnnex),storedName=`angebot-${number}-v${revision}.pdf`,dir=documentationDir(jobId),file=path.join(dir,storedName);await ensureDir(dir);const tmp=file+"."+crypto.randomUUID()+".tmp";try{await fsp.writeFile(tmp,pdf);await fsp.rename(tmp,file)}finally{await fsp.rm(tmp,{force:true}).catch(()=>{})}
...
app.post("/admin/api/job/:jobId/offer-pdf/upload",express.raw({type:["application/pdf","application/octet-stream"],limit:"25mb"}),async(req,res)=>{if(!requireAdmin(req,res))return;try{
  ...
  const draft=await fsp.readFile(offerDraftPath(jobId),"utf8").then(JSON.parse).catch(()=>null);if(!draft||draft.offerNumber!==number||Number(draft.offerRevision||1)!==revision)return res.status(409).json({ok:false,error:"Das Angebot wurde inzwischen geändert. Bitte erneut vorbereiten."});
  if(draft.customerAcceptance?.offerNumber===number&&Number(draft.customerAcceptance?.offerRevision)===revision)return res.status(409).json({ok:false,error:"Dieses Angebot wurde bereits verbindlich beauftragt und darf nicht mehr überschrieben werden."});
  const pdf=await appendOfferLegalAnnex(originalPdf),storedName=`angebot-${number}-v${revision}.pdf`,dir=documentationDir(jobId),file=path.join(dir,storedName);await ensureDir(dir);const tmp=file+"."+crypto.randomUUID()+".tmp";try{await fsp.writeFile(tmp,pdf);await fsp.rename(tmp,file)}finally{await fsp.rm(tmp,{force:true}).catch(()=>{})}
  const rows=await readDocumentation(jobId),id=`offer-${crypto.createHash("sha256").update(`${jobId}|${number}|${revision}`).digest("hex").slice(0,20)}`,item={id,type:"offer",name:`Angebot ${number}.pdf`,offerNumber:number,offerRevision:revision,customerVisible:true,storedName,source:"offer-approved-original",legalAnnexVersion:OFFER_TERMS.version,approvedAt:new Date().toISOString(),importedAt:new Date().toISOString(),url:`/admin/api/job/${encodeURIComponent(jobId)}/documentation/file?name=${encodeURIComponent(storedName)}`},next=rows.filter(row=>row?.id!==id&&row?.storedName!==storedName);next.unshift(item);await writeDocumentation(jobId,next.slice(0,1000));await appendJobHistory(jobId,{type:"offer_pdf_approved",title:`Freigegebene Original-PDF für Angebot ${number} übernommen`,detail:`Version ${revision} · Originalseiten unverändert, AGB und Rücktrittsunterlagen angehängt`,source:"KRISTINE Angebot",data:{number,revision,storedName,originalBytes:originalPdf.length,bytes:pdf.length,legalAnnexVersion:OFFER_TERMS.version}}).catch(()=>{});res.json({ok:true,item,pdfUrl:item.url});
