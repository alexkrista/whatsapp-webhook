...
app.put("/admin/api/job/:jobId/offer-draft", async (req, res) => {
  ...
  try{
    ...
    const measurement={mode:body.measurement?.mode==="estimate"?"estimate":"exact"};for(const key of ["floorArea","wallArea","ceilingArea","roomLength","roomWidth","roomHeight","openingsDeduction","facadeArea","woodArea","undersideArea","perimeter","doors","windows","radiators","hours","months","hourlyRate","regieMaterialRate"])measurement[key]=Math.max(0,Math.min(1000000,Number(body.measurement?.[key]||(key==="hourlyRate"?75:0))));measurement.materialMarkupPct=Math.max(-100,Math.min(10000,Number(body.measurement?.materialMarkupPct??body.measurement?.materialPercent??15)));measurement.regieMaterialMode=body.measurement?.regieMaterialMode==="percent"?"percent":"per_hour";
    ...
    draft.version=5;draft.scopeAuto=body.scopeAuto!==false;draft.priceSnapshotAt=/^\d{4}-\d{2}-\d{2}T/.test(String(body.priceSnapshotAt||""))?String(body.priceSnapshotAt):new Date().toISOString();draft.rooms=(Array.isArray(body.rooms)?body.rooms:[]).slice(0,50).map((r,i)=>({id:String(r?.id||`room-${i+1}`).slice(0,80),name:String(r?.name||`Raum ${i+1}`).trim().slice(0,160),description:String(r?.description||"").trim().slice(0,1500),mode:r?.mode==="estimate"?"estimate":"exact",floorArea:Math.max(0,Math.min(10000,Number(r?.floorArea||0))),length:Math.max(0,Math.min(1000,Number(r?.length||0))),width:Math.max(0,Math.min(1000,Number(r?.width||0))),height:Math.max(0,Math.min(100,Number(r?.height||0))),openingsDeduction:Math.max(0,Math.min(10000,Number(r?.openingsDeduction||0))),includeWalls:r?.includeWalls!==false,includeCeiling:r?.includeCeiling!==false,includeRegie:r?.includeRegie===true,regieHours:Math.max(0,Math.min(10000,Number(r?.regieHours||0))),measureLines:(Array.isArray(r?.measureLines)?r.measureLines:[]).slice(0,50).map(line=>({label:String(line?.label||"").trim().slice(0,160),quantity:Math.max(0,Math.min(1000000,Number(line?.quantity||0))),unit:String(line?.unit||"m²").trim().slice(0,20)})).filter(line=>line.label||line.quantity)}));
    const finance=body.financials||{};draft.financials={priceMode:finance.priceMode==="gross"?"gross":"net",vatRate:Math.max(0,Math.min(100,Number(finance.vatRate??20))),discountPercent:Math.max(0,Math.min(100,Number(finance.discountPercent||0))),skontoEnabled:finance.skontoEnabled===true,skontoPercent:Math.max(0,Math.min(100,Number(finance.skontoPercent||0))),skontoDays:Math.max(0,Math.min(365,Math.round(Number(finance.skontoDays||0))))};
