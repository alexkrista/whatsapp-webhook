...
app.post("/kristool/api/workflows/:id/create-job",async(req,res)=>{
  ...
  try{
    ...
    if(!row)return res.status(404).json({ok:false,error:"Auftragsmappe nicht gefunden."});
    if(row.target!=="order")return res.status(400).json({ok:false,error:"Dieser Vorgang ist kein Auftrag."});
    ...
    if(startDate&&!employeeId)return res.status(400).json({ok:false,error:"Bitte auswählen, wer die Baustelle startet."});
    ...
    const missing=[];if(!customer)missing.push("Kunde/Kontakt");if(!address)missing.push("Baustellenadresse");if(missing.length)return res.status(400).json({ok:false,error:"Vor Auftragserstellung bitte ergänzen: "+missing.join(", ")+"."});
    ...
    await writeJobMeta(jobId,{name:jobName,status:"Auftrag",street:String(streetMatch?.[1]||streetLine).trim(),houseNumber:String(streetMatch?.[2]||"").trim(),postalCode:String(cityMatch?.[1]||"").trim(),city:String(cityMatch?.[2]||cityLine).trim(),addressExtra:address,contactName:customer,contactPhone,contactEmail,customerMaster,wwAddressId:String(customerMaster.wwAddressId||""),wwCustomerNumber:String(customerMaster.wwCustomerNumber||""),customerMasterStatus:customerMaster.wwAddressId?"linked":"pending_ww_create",contractAmount,startDate,notes:notes.slice(0,1000),intakeProtocol:protocol,intakeAppointment:row.appointment||task.appointment||null,intakeTimeline:Array.isArray(row.timeline)?row.timeline:[],sourceWorkflowId:row.id,sourceTaskId:row.taskId,createdAt:new Date().toISOString()});
