...
app.post("/admin/api/job/:jobId/documentation/regie-report",express.raw({type:["application/pdf","application/octet-stream"],limit:"25mb"}),async(req,res)=>{if(!requireAdmin(req,res))return;try{const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});const body=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||"");if(!body.length)return res.status(400).json({ok:false,error:"PDF ist leer"});const dec=key=>{let value=String(req.headers[key]||"");try{value=decodeURIComponent(value)}catch{}return value.trim()},originalName=safeDocumentName(dec("x-file-name"),"Regiebericht.pdf");if(path.extname(originalName).toLowerCase()!==".pdf")return res.status(400).json({ok:false,error:"Bitte einen Regiebericht als PDF verwenden"});const id=`regie-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`,storedName=`${id}.pdf`,dir=documentationDir(jobId);await ensureDir(dir);await fsp.writeFile(path.join(dir,storedName),body);const item={id,type:"regie_report",name:originalName,reportNumber:dec("x-report-number"),employees:dec("x-employees"),totalHours:Math.max(0,Number(dec("x-total-hours").replace(",","."))||0),materialTotal:dec("x-material-total"),reportDate:dec("x-report-date"),importedAt:new Date().toISOString(),storedName,url:`/admin/api/job/${encodeURIComponent(jobId)}/documentation/file?name=${encodeURIComponent(storedName)}`};const rows=await readDocumentation(jobId);rows.unshift(item);await writeDocumentation(jobId,rows.slice(0,1000));await appendJobHistory(jobId,{type:"regie_report_imported",title:`Regiebericht ${item.reportNumber||originalName} importiert`,detail:`${item.employees||"Mitarbeiter offen"} · ${item.totalHours} h · Material ${item.materialTotal||"offen"}`,source:"Dokumentation",data:{reportId:id}}).catch(()=>{});res.status(201).json({ok:true,item})}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
