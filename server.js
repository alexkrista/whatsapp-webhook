...
const BRAIN_PERMIT_PATHS = new Set([
  ...
  "/api/outgoing/project-regie-reports",
...
app.post("/kristool/api/workflows/:id/create-job",async(req,res)=>{
  if(!requireAdmin(req,res))return;
  try{
    const rows=await readVisitWorkflows(),row=rows.find(x=>x.id===String(req.params.id));
    if(!row)return res.status(404).json({ok:false,error:"Angebots-/Auftragsmappe nicht gefunden."});
    if(row.jobId)return res.json({ok:true,jobId:row.jobId,alreadyCreated:true,workflow:row});
...
function sanitizeProjectContacts(value, fallback = {}) {
  ...
  const person = input => {
    ...
    return {
      ...
      uid: text(row.uid, 40), website: text(row.website, 500), sourceUrl: text(row.sourceUrl, 500), sourceCheckedAt: text(row.sourceCheckedAt, 40),
      extraLines: cleanProjectContactExtras(row.extraLines),
    };
  };
  const ownerRole = text(owner.ownerRole, 30) || (/^Firma$/i.test(owner.womanTitle || owner.manTitle || "") ? "Firma" : "");
  const company = ownerRole === "Firma", legacyLast = company ? "" : own(owner, "sharedLastName", own(owner, "customer", legacyMaster.name || fallback.contactName || ""));
  const surname = (field, first) => company && !owner[first] && owner[field] === owner.customer ? "" : own(owner, field, legacyLast);
  ...
  return {
    owner: {
      ...
      uid: text(owner.uid, 40), website: text(owner.website, 500), sourceUrl: text(owner.sourceUrl, 500), sourceCheckedAt: text(owner.sourceCheckedAt, 40),
...
app.put("/admin/api/job/:jobId/offer-draft", async (req, res) => {
  ...
  try{
    const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});
    const body=req.body||{},allowedParts=new Set(["Wände","Decken","Türen","Fenster","Heizkörper","Sockel"]),allowedSteps=new Set(["Grundieren","Mit Dispersion streichen","Acrylfugen","Regie","Material"]),allowedCover=new Set(["Boden und Laufwege","Fußleisten","Fenster und Türen","Mobiliar","Abdeckarbeiten pauschal"]),allowedFacade=new Set(["Gerüst","Baustelleneinrichtung / Miet-WC","Abdeckarbeiten","Fassade waschen","Algen- und Pilzreiniger","Silikonharzgrundierung","Fassadenfarbe 2x","Untersicht reinigen und schleifen","Jotun Trebitt Grundierung","Deckender Holzanstrich 2x","Holz reinigen und schleifen","Holz imprägnieren","Holzlasur 2x","Regie","Material"]),allowedBathroom=new Set(["Bad einrichten und abdecken","Fliesenabsatz ausgleichen","Alte Fliesen schleifen und reinigen","Untergrund vorbereiten","Wände und Böden vorbereiten","Spezialspachtel glätten","Dusche und Boden abdichten","Duschrinne oder Gully anarbeiten","Duschwand herstellen","Gefälleestrich herstellen","Duschnische ausbilden","Sitzbank mauern","Terra Icon Unikatbelag","Aluschienen","Silikonfugen","Anfahrten"]),allowedBathroomCompact=new Set(["Bad einrichten und abdecken","Untergrund vorbereiten","Aluschienen","Gefälleestrich herstellen","Duschnische ausbilden","Dusche und Boden abdichten","Duschrinne oder Gully anarbeiten","Silikonfugen","Terra Icon Unikatbelag","Anfahrten"]),allowedLime=new Set(["Boden und Laufwege","Fußleisten","Fenster und Türen","Mobiliar","Abdeckarbeiten pauschal","Kalk-Haftbrücke mit Gewebe","Sumpfkalk-Wohlfühlputz","Edelstahlkantenschutz","Regie","Material"]),allowedWdvs=new Set(["Abdecken außen","Isolierung kleben","Dübeln","Kantenprofile setzen","Spachteln","Musterputz und Grundierung","Fassade schleifen","Abdichtung / Optiflex","Fenster schneiden"]);
...
const renderJobDocumentPdf=createJobDocumentPdfRenderer({
  renderPdf:renderOfferHtmlPdf,
  readJobMeta,
  isSafeJobId,
  jobExists:jobId=>fsp.stat(metaPathForJob(jobId)).then(stat=>stat.isFile()).catch(()=>false),
});
const documentLayout=require("./document-layout").registerDocumentLayout(app,{dataDir:DATA_DIR,requireAdmin,publicDir:path.join(process.cwd(),"public"),renderPdf:renderJobDocumentPdf});
...
require("./order-confirmation").registerOrderConfirmation(app, {
  ...
  readDocumentation, writeDocumentation, appendJobHistory, renderPdf: renderJobDocumentPdf, readDocumentLayout:documentLayout.read,
});
...
app.post("/admin/api/job/:jobId/offer-pdf/render",express.text({type:"text/html",limit:"5mb"}),async(req,res)=>{if(!requireAdmin(req,res))return;try{
  ...
  if(draft.customerAcceptance?.offerNumber===number&&Number(draft.customerAcceptance?.offerRevision)===revision)return res.status(409).json({ok:false,error:"Dieses Angebot wurde bereits verbindlich beauftragt und darf nicht mehr überschrieben werden."});
  const pdf=await renderJobDocumentPdf(req.body,{jobId}),storedName=`angebot-${number}-v${revision}.pdf`,dir=documentationDir(jobId),file=path.join(dir,storedName),immutableDir=path.join(DATA_DIR,jobId,"_offers"),immutableFile=path.join(immutableDir,storedName);await ensureDir(dir);await ensureDir(immutableDir);const tmp=file+"."+crypto.randomUUID()+".tmp";try{await fsp.writeFile(tmp,pdf);await fsp.rename(tmp,file);await fsp.copyFile(file,immutableFile)}finally{await fsp.rm(tmp,{force:true}).catch(()=>{})}
  const rows=await readDocumentation(jobId),id=`offer-${crypto.createHash("sha256").update(`${jobId}|${number}|${revision}`).digest("hex").slice(0,20)}`,item={id,type:"offer",name:`Angebot ${number}.pdf`,offerNumber:number,offerRevision:revision,customerVisible:true,storedName,source:"offer-browser-render",termsVersion:OFFER_TERMS.version,renderedAt:new Date().toISOString(),importedAt:new Date().toISOString(),url:`/admin/api/job/${encodeURIComponent(jobId)}/documentation/file?name=${encodeURIComponent(storedName)}`},next=rows.filter(row=>row?.id!==id&&row?.storedName!==storedName);next.unshift(item);await writeDocumentation(jobId,next.slice(0,1000));await appendJobHistory(jobId,{type:"offer_pdf_stored",title:`Angebot ${number} als verbindliche PDF gespeichert`,detail:`Version ${revision} · identisch für Druck, Versand, Dokumente und Kundenportal · AGB separat im Kundenportal bestätigt`,source:"KRISTINE Angebot",data:{number,revision,storedName,termsVersion:OFFER_TERMS.version}}).catch(()=>{});res.json({ok:true,item,pdfUrl:item.url});
}catch(e){res.status(e.status||500).json({ok:false,error:`PDF konnte nicht verbindlich gespeichert werden: ${String(e?.message||e)}`})}});
...
app.post("/admin/api/job/:jobId/offer-pdf/upload",express.raw({type:["application/pdf","application/octet-stream"],limit:"25mb"}),async(req,res)=>{if(!requireAdmin(req,res))return;try{
  const jobId=String(req.params.jobId||""),number=String(req.headers["x-offer-number"]||"").trim(),revision=Math.max(1,Number(req.headers["x-offer-revision"]||1)),originalPdf=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});if(originalPdf.length<100||originalPdf.subarray(0,5).toString("ascii")!=="%PDF-")return res.status(400).json({ok:false,error:"Bitte die freigegebene Original-PDF auswählen."});
  const draft=await fsp.readFile(offerDraftPath(jobId),"utf8").then(JSON.parse).catch(()=>null);if(!draft||draft.offerNumber!==number||Number(draft.offerRevision||1)!==revision)return res.status(409).json({ok:false,error:"Das Angebot wurde inzwischen geändert. Bitte erneut vorbereiten."});
  const accepted=draft.customerAcceptance?.offerNumber===number&&Number(draft.customerAcceptance?.offerRevision)===revision,approvedCorrection=String(req.headers["x-approved-pdf-correction"]||"")==="1";if(accepted&&!approvedCorrection)return res.status(409).json({ok:false,error:"Dieses Angebot wurde bereits beauftragt. Für die dokumentierte PDF-Korrektur bitte ausdrücklich ‚freigegebene PDF korrigieren‘ verwenden."});
...
async function markRegieReportsBilledByInvoice(jobId, reportIds, invoice = {}) {
  ...
  const rows = await readDocumentation(jobId), changed = [], now = new Date().toISOString();
  const updated = rows.map(row => {
    if (row?.type !== "regie_report" || !wanted.has(String(row.id || "")) || row.billingStatus === "billed") return row;
    const entry = { reportId: String(row.id || ""), reportNumber: String(row.reportNumber || row.name || ""), invoiceNumber: String(invoice.invoiceNumber || "") };
    changed.push(entry);
    return { ...row, billingStatus:"billed", billingStatusChangedAt:now, billedByKristineInvoice:{ invoiceId:Number(invoice.id)||0, invoiceNumber:entry.invoiceNumber, kind:String(invoice.kind||""), issueDate:String(invoice.issueDate||"").slice(0,10) } };
  });
  if (!changed.length) return { changed: 0, reports: [] };
  await writeDocumentation(jobId, updated);
  await appendJobHistory(jobId, { type:"regie_reports_billed_by_invoice", title:`${changed.length} Regiebericht(e) automatisch abgerechnet`, detail:invoice.invoiceNumber?`Mit KRISTINE-Rechnung ${String(invoice.invoiceNumber)} abgeschlossen.`:"Mit einer ausgestellten KRISTINE-Rechnung abgeschlossen.", source:"KRISTINE", data:{reports:changed} }).catch(()=>{});
  return { changed: changed.length, reports: changed };
}
function regieReportSequence(row) { const text = String(row?.sheetNumber || row?.reportNumber || row?.name || "").trim(); const match = text.match(/\/(\d+)\D*$/) || text.match(/(?:^|\D)(\d{1,3})\D*$/); return match ? String(Number(match[1])) : ""; }
function regieReportMatchKey(row) { const date = String(row?.reportDate || "").slice(0, 10), sequence = regieReportSequence(row); return date && sequence ? `${date}|${sequence}` : ""; }
async function removeDuplicateRegiePdfRows(jobId, rows) {
  const wwKeys = new Set(rows.filter(row => row?.type === "regie_report" && String(row?.source || "").toUpperCase() === "WW").map(regieReportMatchKey).filter(Boolean));
  const removed = [], kept = rows.filter(row => { const duplicate = row?.type === "regie_report" && String(row?.source || "").toUpperCase() !== "WW" && wwKeys.has(regieReportMatchKey(row)); if (duplicate) removed.push(row); return !duplicate; });
  const referencedFiles = new Set(kept.map(row => String(row?.storedName || "")).filter(Boolean));
  const removableFiles = [...new Set(removed.map(row => String(row?.storedName || "")).filter(name => name && !referencedFiles.has(name)))];
  for (const name of removableFiles) await fsp.unlink(path.join(documentationDir(jobId), safeDocumentName(name))).catch(() => {});
  return { rows: kept, removedCount: removed.length, removedFiles: removableFiles.length };
}
function safeDocumentName(value, fallback="Datei") { return (path.basename(String(value || fallback)).replace(/[\x00-\x1f<>:"/\\|?*]+/g, "_").trim() || fallback).slice(0, 180); }
function emlHeader(text, name) { const match=String(text||"").match(new RegExp(`^${name}:\\s*([^\\r\\n]*(?:\\r?\\n[ \\t]+[^\\r\\n]*)*)`,"im")); return String(match?.[1]||"").replace(/\r?\n[ \t]+/g," ").trim(); }
function emlAttachments(buffer) { const text=buffer.toString("latin1"),rows=[];for(const part of text.split(/\r?\n--[-A-Za-z0-9_'()+,.\/:=? ]+/)){const nameMatch=part.match(/filename\*?=(?:UTF-8''|)(?:"([^"]+)"|([^;\r\n]+))/i);if(!nameMatch)continue;let decoded=String(nameMatch[1]||nameMatch[2]||"").trim();try{decoded=decodeURIComponent(decoded)}catch{}const name=safeDocumentName(decoded.replace(/%20/g," "),"Anlage"),split=part.search(/\r?\n\r?\n/);if(split<0)continue;const body=part.slice(split).replace(/^\r?\n\r?\n/,"").replace(/\r?\n--.*$/s,"").trim(),encoding=(part.match(/Content-Transfer-Encoding:\s*([^\r\n]+)/i)?.[1]||"").toLowerCase();try{const content=encoding.includes("base64")?Buffer.from(body.replace(/\s/g,""),"base64"):Buffer.from(body,"latin1");if(content.length)rows.push({name,content})}catch{}}return rows.slice(0,30); }

app.get("/admin/api/job/:jobId/documentation", async (req,res)=>{if(!requireAdmin(req,res))return;const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});res.json({ok:true,jobId,items:await readDocumentation(jobId)})});
app.post("/admin/api/job/:jobId/documentation/regie-reports-billed",async(req,res)=>{if(!requireAdmin(req,res))return;try{const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});const result=await markRegieReportsBilledByInvoice(jobId,req.body?.reportIds,{id:req.body?.invoiceId,invoiceNumber:String(req.body?.invoiceNumber||"").slice(0,80),kind:String(req.body?.kind||"").slice(0,8),issueDate:String(req.body?.issueDate||"").slice(0,10)});res.json({ok:true,...result})}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
app.post("/admin/api/job/:jobId/documentation/mail", express.raw({type:["message/rfc822","application/vnd.ms-outlook","application/octet-stream"],limit:"25mb"}), async(req,res)=>{if(!requireAdmin(req,res))return;try{const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});const body=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||"");if(!body.length)return res.status(400).json({ok:false,error:"Maildatei ist leer"});let decodedName=String(req.headers["x-file-name"]||"Mail.eml");try{decodedName=decodeURIComponent(decodedName)}catch{}const originalName=safeDocumentName(decodedName),ext=path.extname(originalName).toLowerCase();if(![".msg",".eml"].includes(ext))return res.status(400).json({ok:false,error:"Bitte eine .msg- oder .eml-Datei verwenden"});const id=`mail-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`,dir=documentationDir(jobId);await ensureDir(dir);const storedOriginal=`${id}${ext}`;await fsp.writeFile(path.join(dir,storedOriginal),body);let mail={},attachments=[];if(ext===".msg"){const parsed=parseMsg(body);mail=parsed.mail||{};for(const meta of mail.attachments||[]){const attachment=getMsgAttachment(body,meta.index);if(attachment?.content?.length)attachments.push(attachment)}}else{const text=body.toString("utf8");mail={subject:emlHeader(text,"Subject"),senderEmail:emlHeader(text,"From"),to:emlHeader(text,"To"),sentAt:emlHeader(text,"Date")};attachments=emlAttachments(body)}const savedAttachments=[];for(let i=0;i<attachments.length;i++){const attachment=attachments[i],name=safeDocumentName(attachment.name,`Anlage-${i+1}`),stored=`${id}-a${i}-${name}`;await fsp.writeFile(path.join(dir,stored),attachment.content);const plan=/\.(pdf|dwg|dxf|ifc|plt|dgn)$/i.test(name);savedAttachments.push({name,storedName:stored,kind:plan?"plan":"attachment",size:attachment.content.length,url:`/admin/api/job/${encodeURIComponent(jobId)}/documentation/file?name=${encodeURIComponent(stored)}`})}const item={id,type:"mail",name:originalName,subject:String(mail.subject||originalName),fromName:String(mail.senderName||""),fromEmail:String(mail.senderEmail||""),from:String(mail.senderName||mail.senderEmail||""),to:Array.isArray(mail.to)?mail.to.map(x=>x.label||x.email||x.name).filter(Boolean).join("; "):String(mail.to||""),sentAt:String(mail.sentAt||""),importedAt:new Date().toISOString(),storedName:storedOriginal,url:`/admin/api/job/${encodeURIComponent(jobId)}/documentation/file?name=${encodeURIComponent(storedOriginal)}`,attachments:savedAttachments};const rows=await readDocumentation(jobId);rows.unshift(item);await writeDocumentation(jobId,rows.slice(0,1000));await appendJobHistory(jobId,{type:"mail_imported",title:`Mail abgelegt: ${item.subject}`,detail:`${savedAttachments.length} Anlage(n), davon ${savedAttachments.filter(x=>x.kind==="plan").length} Plan/Pläne`,source:"Dokumentation",data:{mailId:id}}).catch(()=>{});res.status(201).json({ok:true,item})}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
app.get("/admin/api/job/:jobId/documentation/file",async(req,res)=>{if(!requireAdmin(req,res))return;const jobId=String(req.params.jobId||""),name=safeDocumentName(req.query.name||"");if(!isSafeJobId(jobId)||!name)return res.status(400).send("Ungültige Datei");const file=path.join(documentationDir(jobId),name);if(!fs.existsSync(file))return res.status(404).send("Datei nicht gefunden");res.set("X-Content-Type-Options","nosniff");if(/\.(pdf|png|jpe?g|gif|webp)$/i.test(name))res.sendFile(file);else res.download(file,name)});
app.post("/admin/api/job/:jobId/documentation/regie-comparison.pdf",async(req,res)=>{if(!requireAdmin(req,res))return;try{const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});const rows=(Array.isArray(req.body?.rows)?req.body.rows:[]).slice(0,2000).map(x=>({date:String(x.date||"").slice(0,10),dateLabel:String(x.dateLabel||"").slice(0,20),name:String(x.name||"").slice(0,120),stamped:Number(x.stamped)||0,regie:x.regie===null?null:(Number(x.regie)||0),diff:x.diff===null?null:(Number(x.diff)||0),number:String(x.number||"").slice(0,80),noReport:!!x.noReport})),payload={jobId,jobName:String(req.body?.jobName||"").slice(0,160),rows,totalStamped:rows.reduce((s,x)=>s+x.stamped,0),totalRegie:rows.reduce((s,x)=>s+(x.regie===null?0:x.regie),0),totalDiff:rows.reduce((s,x)=>s+(x.diff===null?0:x.diff),0),laborTotal:Number(req.body?.laborTotal)||0,materialTotal:Number(req.body?.materialTotal)||0},pdf=await createRegieComparisonPdf(payload);res.setHeader("Content-Type","application/pdf");res.setHeader("Content-Disposition",`attachment; filename="Stundenabgleich_${safeDocumentName(jobId)}.pdf"`);res.send(pdf)}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
app.post("/admin/api/job/:jobId/documentation/regie.xlsx",async(req,res)=>{if(!requireAdmin(req,res))return;try{if(!XLSX)throw new Error("Excel-Export ist auf dem Server derzeit nicht verfügbar");const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});const clean=(value,max=300)=>String(value??"").trim().slice(0,max),safeNumber=(value,max)=>{const number=Number(value);return Number.isFinite(number)?Math.max(0,Math.min(max,number)):0},rows=(Array.isArray(req.body?.rows)?req.body.rows:[]).slice(0,5000).map(x=>{const dateText=clean(x.date,10),date=/^\d{4}-\d{2}-\d{2}$/.test(dateText)?new Date(`${dateText}T12:00:00Z`):"";return [clean(x.number,120),date,clean(x.people,500),safeNumber(x.hours,100000),safeNumber(x.materialValue,100000000),clean(x.status,120)]}),worksheet=XLSX.utils.aoa_to_sheet([["Nr.","Datum","Wer","Std.","Materialwert","Status"],...rows],{cellDates:true});worksheet["!cols"]=[{wch:18},{wch:13},{wch:42},{wch:12},{wch:17},{wch:28}];worksheet["!autofilter"]={ref:worksheet["!ref"]||"A1:F1"};for(let index=2;index<=rows.length+1;index++){if(worksheet[`B${index}`])worksheet[`B${index}`].z="dd.mm.yyyy";if(worksheet[`D${index}`])worksheet[`D${index}`].z="0.00";if(worksheet[`E${index}`])worksheet[`E${index}`].z='#,##0.00 [$€-407]'}const workbook=XLSX.utils.book_new();XLSX.utils.book_append_sheet(workbook,worksheet,"Regieberichte");const buffer=XLSX.write(workbook,{type:"buffer",bookType:"xlsx"});res.setHeader("Content-Type","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");res.setHeader("Content-Disposition",`attachment; filename="Regieberichte_${safeDocumentName(jobId)}.xlsx"`);res.send(buffer)}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
app.post("/admin/api/job/:jobId/economy/gross-profit.pdf",async(req,res)=>{if(!requireAdmin(req,res))return;try{const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});const safeNumber=(value,max=100000000)=>Math.max(0,Math.min(max,Number(value)||0)),rows=(Array.isArray(req.body?.rows)?req.body.rows:[]).slice(0,200).map(row=>{const hours=safeNumber(row?.hours,1000000),wageRate=safeNumber(row?.wageRate,10000),gkRate=safeNumber(row?.gkRate,10000),wageCost=hours*wageRate,gkCost=hours*gkRate;return{name:String(row?.name||"Unbekannt").trim().slice(0,120),hours,wageRate,wageCost,gkRate,gkCost,totalCost:wageCost+gkCost,matched:row?.matched!==false}}),wageTotal=rows.reduce((sum,row)=>sum+row.wageCost,0),gkTotal=rows.reduce((sum,row)=>sum+row.gkCost,0),employeeTotal=wageTotal+gkTotal,documentNet=safeNumber(req.body?.documentNet),materialEk=safeNumber(req.body?.materialEk),payload={jobId,jobName:String(req.body?.jobName||"").trim().slice(0,160),createdAt:new Date().toLocaleString("de-AT",{timeZone:"Europe/Vienna"}),rows,wageTotal,gkTotal,employeeTotal,documentNet,materialEk,grossProfit:documentNet-employeeTotal-materialEk},pdf=await createGrossProfitPdf(payload);res.setHeader("Content-Type","application/pdf");res.setHeader("Content-Disposition",`attachment; filename="Ertrag_${safeDocumentName(jobId)}.pdf"`);res.send(pdf)}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
app.post("/admin/api/job/:jobId/documentation/regie-report-sync",async(req,res)=>{if(!requireAdmin(req,res))return;try{const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});const cleanText=(value,max=500)=>String(value||"").trim().slice(0,max),cleanNumber=(value,min=0,max=10000000)=>Math.max(min,Math.min(max,Number(value)||0)),cleanDate=value=>/^\d{4}-\d{2}-\d{2}$/.test(String(value||"").slice(0,10))?String(value).slice(0,10):"",cleanTime=value=>/^([01]\d|2[0-3]):[0-5]\d$/.test(String(value||""))?String(value):"",incoming=(Array.isArray(req.body?.reports)?req.body.reports:[]).map(raw=>{const sourceId=cleanText(raw.sourceId,80),reportNumber=cleanText(raw.reportNumber||raw.name,120),employeeDetails=(Array.isArray(raw.employeeDetails)?raw.employeeDetails:[]).slice(0,100).map(person=>({employeeIndex:Math.trunc(cleanNumber(person.employeeIndex,0,10000000)),employeeNumber:cleanText(person.employeeNumber,40),name:cleanText(person.name,160),hours:cleanNumber(person.hours,0,1000),cost:cleanNumber(person.cost),unitPrice:cleanNumber(person.unitPrice,0,10000),from:cleanTime(person.from),to:cleanTime(person.to)})),materials=(Array.isArray(raw.materials)?raw.materials:[]).slice(0,500).map(material=>({sourceId:cleanText(material.sourceId,80),materialIndex:Math.trunc(cleanNumber(material.materialIndex,0,100000000)),name:cleanText(material.name||"Material",250),quantity:cleanNumber(material.quantity,0,1000000),unit:cleanText(material.unit,20),unitPrice:cleanNumber(material.unitPrice),cost:cleanNumber(material.cost),purchaseUnitPrice:cleanNumber(material.purchaseUnitPrice),purchaseCost:cleanNumber(material.purchaseCost),markupPercent:cleanNumber(material.markupPercent,0,10000),note:cleanText(material.note,1000)}));return {id:`ww-regie-${crypto.createHash("sha1").update(sourceId||`${jobId}|${reportNumber}|${raw.reportDate||""}`).digest("hex").slice(0,20)}`,type:"regie_report",source:"WW",projectNumber:cleanText(raw.projectNumber||jobId,40),sourceId,projectNumber:jobId,reportNumber,name:cleanText(raw.name||reportNumber,120),sheetNumber:cleanText(raw.sheetNumber,20),reportDate:cleanDate(raw.reportDate),changedAt:cleanText(raw.changedAt,40),statusCode:Math.trunc(cleanNumber(raw.statusCode,0,1000)),statusLabel:cleanText(raw.statusLabel,80),description:cleanText(raw.description,12000),note:cleanText(raw.note,3000),materialFlatPercent:cleanNumber(raw.materialFlatPercent,0,10000),billedDocumentId:cleanText(raw.billedDocumentId,80),billedDocumentType:Math.trunc(cleanNumber(raw.billedDocumentType,0,1000)),employees:employeeDetails.map(x=>x.name).filter(Boolean).join(", "),employeeDetails,materials,totalHours:cleanNumber(raw.totalHours,0,100000),laborCost:cleanNumber(raw.laborCost),materialCost:cleanNumber(raw.materialCost),materialTotal:cleanNumber(raw.materialCost??raw.materialTotal),totalNet:cleanNumber(raw.totalNet),syncedAt:new Date().toISOString()}}).filter(x=>x.sourceId&&x.reportNumber),rows=await readDocumentation(jobId),key=value=>String(value||"").toLowerCase().replace(/[^a-z0-9]+/g,""),created=[],updated=[];for(const item of incoming){let index=rows.findIndex(row=>row.type==="regie_report"&&row.source==="WW"&&String(row.sourceId||"")===item.sourceId);if(index<0)index=rows.findIndex(row=>row.type==="regie_report"&&(!row.projectNumber||row.projectNumber===item.projectNumber)&&key(row.reportNumber)===key(item.reportNumber));if(index>=0){const old=rows[index];rows[index]={...old,...item,id:old.id||item.id,storedName:old.storedName,url:old.url,sourceSummaryId:old.sourceSummaryId,importedAt:old.importedAt||item.syncedAt};updated.push(rows[index])}else{item.importedAt=item.syncedAt;rows.unshift(item);created.push(item)}}await writeDocumentation(jobId,rows);if(incoming.length)await appendJobHistory(jobId,{type:"ww_regie_synced",title:`${incoming.length} WinWorker-Rapport(e) synchronisiert`,detail:`${created.length} neu · ${updated.length} aktualisiert · Tätigkeit, Stunden, Material und Preise übernommen`,source:"WinWorker",data:{created:created.length,updated:updated.length,sourceIds:incoming.map(x=>x.sourceId)}}).catch(()=>{});res.json({ok:true,count:incoming.length,created:created.length,updated:updated.length,items:incoming})}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
app.post("/admin/api/job/:jobId/documentation/regie-report-deduplicate",async(req,res)=>{if(!requireAdmin(req,res))return;try{const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});const cleanup=await removeDuplicateRegiePdfRows(jobId,await readDocumentation(jobId));await writeDocumentation(jobId,cleanup.rows.slice(0,1000));if(cleanup.removedCount)await appendJobHistory(jobId,{type:"regie_pdf_duplicates_removed",title:`${cleanup.removedCount} PDF-Dublette(n) entfernt`,detail:"Der zugehörige WinWorker-Rapport bleibt vollständig erhalten.",source:"Dokumentation",data:cleanup}).catch(()=>{});res.json({ok:true,...cleanup,rows:undefined})}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
app.patch("/admin/api/job/:jobId/documentation/regie-report/:reportId/status",async(req,res)=>{if(!requireAdmin(req,res))return;try{const jobId=String(req.params.jobId||""),reportId=String(req.params.reportId||"");if(!isSafeJobId(jobId)||!reportId||reportId.length>160)return res.status(400).json({ok:false,error:"Ungültiger Regiebericht."});const billingStatus=String(req.body?.billingStatus||"").toLowerCase();if(!["","open","billed"].includes(billingStatus))return res.status(400).json({ok:false,error:"Ungültiger Abrechnungsstatus."});const rows=await readDocumentation(jobId),index=rows.findIndex(row=>row?.type==="regie_report"&&String(row.id||"")===reportId);if(index<0)return res.status(404).json({ok:false,error:"Regiebericht nicht gefunden."});if(billingStatus)rows[index]={...rows[index],billingStatus,billingStatusChangedAt:new Date().toISOString()};else{rows[index]={...rows[index]};delete rows[index].billingStatus;delete rows[index].billingStatusChangedAt}await writeDocumentation(jobId,rows);await appendJobHistory(jobId,{type:"regie_billing_status_changed",title:`Regiebericht ${rows[index].reportNumber||rows[index].name||reportId}: ${billingStatus==="billed"?"abgerechnet":billingStatus==="open"?"offen":"automatisch"}`,detail:"Der Abrechnungsstatus wurde in KRISTINE festgelegt.",source:"admin",data:{reportId,billingStatus}}).catch(()=>{});res.json({ok:true,item:rows[index]})}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
app.post("/admin/api/job/:jobId/documentation/regie-report",express.raw({type:["application/pdf","application/octet-stream"],limit:"25mb"}),async(req,res)=>{if(!requireAdmin(req,res))return;try{const jobId=String(req.params.jobId||"");if(!isSafeJobId(jobId))return res.status(400).json({ok:false,error:"Invalid jobId"});const body=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||"");if(!body.length)return res.status(400).json({ok:false,error:"PDF ist leer"});const dec=key=>{let value=String(req.headers[key]||"");try{value=decodeURIComponent(value)}catch{}return value.trim()},originalName=safeDocumentName(dec("x-file-name"),"Regiebericht.pdf");if(path.extname(originalName).toLowerCase()!==".pdf")return res.status(400).json({ok:false,error:"Bitte einen Regiebericht als PDF verwenden"});const id=`regie-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`,storedName=`${id}.pdf`,dir=documentationDir(jobId),importedAt=new Date().toISOString(),url=`/admin/api/job/${encodeURIComponent(jobId)}/documentation/file?name=${encodeURIComponent(storedName)}`;await ensureDir(dir);await fsp.writeFile(path.join(dir,storedName),body);let extracted=[];try{extracted=await extractRegieReportsFromPdf(body)}catch(parseError){console.error("regie_summary_parse_error",{jobId,error:String(parseError?.message||parseError)})}const items=extracted.length>1?extracted.map((row,index)=>({id:`${id}-${index+1}`,type:"regie_report",name:originalName,...row,importedAt,storedName,url,sourceSummaryId:id})): [{id,type:"regie_report",name:originalName,reportNumber:extracted[0]?.reportNumber||dec("x-report-number"),employees:extracted[0]?.employees||dec("x-employees"),totalHours:extracted[0]?.totalHours||Math.max(0,Number(dec("x-total-hours").replace(",","."))||0),materialTotal:extracted[0]?.materialTotal||dec("x-material-total"),reportDate:extracted[0]?.reportDate||dec("x-report-date"),importedAt,storedName,url}];let rows=await readDocumentation(jobId);if(extracted.length>1)rows=rows.filter(x=>!(x.type==="regie_report"&&x.name===originalName&&!x.sourceSummaryId&&!Array.isArray(x.employeeDetails)));const known=new Set(rows.filter(x=>x.type==="regie_report").map(x=>String(x.reportNumber||"")).filter(Boolean)),fresh=items.filter(x=>!x.reportNumber||!known.has(String(x.reportNumber)));rows.unshift(...fresh);await writeDocumentation(jobId,rows.slice(0,1000));await appendJobHistory(jobId,{type:"regie_report_imported",title:extracted.length>1?`${fresh.length} Rapporte aus ${originalName} importiert`:`Regiebericht ${fresh[0]?.reportNumber||originalName} importiert`,detail:extracted.length>1?`Sammelaufstellung automatisch getrennt · ${items.length-fresh.length} Duplikat(e) übersprungen`:`${fresh[0]?.employees||"Mitarbeiter offen"} · ${fresh[0]?.totalHours||0} h · Material ${fresh[0]?.materialTotal||"offen"}`,source:"Dokumentation",data:{reportId:id,count:fresh.length}}).catch(()=>{});res.status(201).json({ok:true,item:fresh[0]||null,items:fresh,extracted:extracted.length,duplicates:items.length-fresh.length})}catch(error){res.status(500).json({ok:false,error:String(error?.message||error)})}});
...
registerOutgoingBillingBridge(app, {
  ...
  onIssuedProgressInvoices: async ({ jobId, invoices }) => {
    for (const invoice of Array.isArray(invoices) ? invoices : []) {
      if (invoice.progressBilling?.jobId && String(invoice.progressBilling.jobId) !== String(jobId)) continue;
      await markRegieReportsBilledByInvoice(jobId, invoice.progressBilling?.reportIdsToBill, invoice);
    }
  },
});
...
const dailyReport = registerDailyReport(app, {
  ...
  publicBaseUrl: PUBLIC_BASE_URL,
  adminToken: ADMIN_TOKEN,
  sendMail: sendMailWithLink,
  mailTo: MAIL_TO_DEFAULT,
  logger: console,
});
console.log("✅ KRISTA Tagesreport + Tageszusammenfassung aktiv");
...
app.post("/admin/api/morning-status/test", async (req, res) => {
  ...
  try {
    ...
    if (["640", "645", "7"].includes(type) && !employeeId) {
    ...
    const result =
      ["640", "645"].includes(type)
        ? await morningStatus.runSixFortyFive(date, true, employeeId)
        : type === "7"
          ? await morningStatus.runSevenOClock(date, true, employeeId)
          : type === "15"
            ? await morningStatus.runFifteenOClock(date, true)
            : type === "1530"
              ? await morningStatus.runFifteenThirty(date, true)
...
// ===================== Teleport / Scheduler Diagnose =====================
app.get("/admin/api/teleport/status", async (req, res) => {
  if (!requireAdmin(req, res)) return;
  try {
    res.json({
      ok: true,
      generatedAt: new Date().toISOString(),
      chefPhoneConfigured: Boolean(normalizeWhatsAppRecipient(CHEF_PHONE)),
      activeSenderIdConfigured: Boolean(getActiveKristinePhoneNumberId()),
      morning: morningStatus?.getStatus
        ? await morningStatus.getStatus()
        : { ok: false, error: "Morgenstatus noch nicht bereit" },
      dailyReport: dailyReport?.getStatus
        ? await dailyReport.getStatus()
        : { ok: false, error: "Tagesreport noch nicht bereit" },
    });
  } catch (error) {
    res.status(500).json({
      ok: false,
