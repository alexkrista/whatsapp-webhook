...
function Invoke-MysqlRaw {
  ...
  # Windows PowerShell 5.1 wandelt STDERR von nativen Programmen in ErrorRecords um.
  # Bei $ErrorActionPreference='Stop' wurde dadurch schon vor unserer Access-Denied-
  # Behandlung abgebrochen. Fuer genau diesen nativen Aufruf schalten wir daher
  # temporaer auf Continue und werten Exitcode + Ausgabe selbst aus.
  ...
  try {
    ...
    if ($hadMysqlPwd) { $env:MYSQL_PWD = $oldMysqlPwd }
  ...
  if ($AllowPrompt -and $message -match "Access denied") {
...
foreach ($row in $tableRows) {
  ...
  $rowCount = 0
  if ($parts.Count -gt 1) { [void][long]::TryParse([string]$parts[1], [ref]$rowCount) }
  $tableInfo += [pscustomobject]@{ Table = [string]$parts[0]; Rows = [long]$rowCount }
...
foreach ($row in $columnRows) {
  ...
  if (-not $columnsByTable.ContainsKey($tableName)) { $columnsByTable[$tableName] = @() }
  $position = 0
  if ($parts.Count -gt 3) { [void][int]::TryParse([string]$parts[3], [ref]$position) }
  $columnsByTable[$tableName] += [pscustomobject]@{ Name = [string]$parts[1]; Type = [string]$parts[2]; Pos = [int]$position }
}
...
function Get-CandidateScore {
  ...
  $score = 0
  if ($TableName -match '(?i)history|order|dispens|tint|transaction|job|ticket|sale|queue|batch|mix') { $score += 12 }
...
foreach ($tableEntry in $tableInfo) {
  ...
  $cols = @()
  if ($columnsByTable.ContainsKey($tableName)) { $cols = @($columnsByTable[$tableName]) }
  $score = Get-CandidateScore -TableName $tableName -Columns $cols
  if ($score -gt 0) {
    $candidates += [pscustomobject]@{ Table = $tableName; Rows = [long]$tableEntry.Rows; Score = [int]$score; Columns = $cols }
  }
}
...
foreach ($candidate in @($candidates | Select-Object -First 30)) {
  $columnDescriptions = @()
  foreach ($column in $candidate.Columns) { $columnDescriptions += ([string]$column.Name + ":" + [string]$column.Type) }
...
foreach ($candidate in @($candidates | Select-Object -First 12)) {
  ...
  $dateCol = $null
  foreach ($column in $safeCols) {
    if ($column.Type -match '(?i)datetime|timestamp|date' -and $column.Name -match '(?i)dispens|tint|complete|modified|created|date|time|timestamp') { $dateCol = $column; break }
  }
  if ($null -eq $dateCol) {
    foreach ($column in $safeCols) { if ($column.Type -match '(?i)datetime|timestamp|date') { $dateCol = $column; break } }
  }
  $orderSql = ""
  if ($null -ne $dateCol) { $orderSql = " ORDER BY " + (Sql-Identifier -Name ([string]$dateCol.Name)) + " DESC" }
  ...
  try {
    ...
    if ($samples.Count -eq 0) { Add-Line "  (empty)" }
    foreach ($sample in $samples) { Add-Line ("  " + [string]$sample) }
  }
...
if ($needles.Count -gt 0) {
  ...
  foreach ($tableName in $columnsByTable.Keys) {
    foreach ($column in @($columnsByTable[$tableName] | Where-Object { $_.Type -match '(?i)char|text|enum|set' })) {
      $quotedTable = Sql-Identifier -Name ([string]$tableName)
      $quotedColumn = Sql-Identifier -Name ([string]$column.Name)
      $conditions = @()
      foreach ($needle in $needles) { $conditions += ($quotedColumn + " LIKE '%" + (Sql-Escape -Value ([string]$needle)) + "%'") }
      try {
        $countSql = "SELECT COUNT(*) FROM " + $quotedTable + " WHERE " + ($conditions -join " OR ") + ";"
        $countRows = @(Invoke-Mysql -Sql $countSql)
        $count = 0
        if ($countRows.Count -gt 0) { [void][int]::TryParse([string]$countRows[0], [ref]$count) }
        if ($count -gt 0) {
          ...
          foreach ($sample in @(Invoke-Mysql -Sql $sampleSql)) { Add-Line ("  " + [string]$sample) }
        }
      }
      catch {}
    }
...
if (-not [string]::IsNullOrWhiteSpace($HistoryTime)) {
  ...
  foreach ($tableName in $columnsByTable.Keys) {
    foreach ($column in @($columnsByTable[$tableName] | Where-Object { $_.Type -match '(?i)datetime|timestamp' })) {
      try {
        $quotedTable = Sql-Identifier -Name ([string]$tableName)
        $quotedColumn = Sql-Identifier -Name ([string]$column.Name)
        ...
        $count = 0
        if ($countRows.Count -gt 0) { [void][int]::TryParse([string]$countRows[0], [ref]$count) }
        if ($count -gt 0) { Add-Line ("TIME " + $tableName + "." + $column.Name + " - " + $count + " row(s)") }
      }
      catch {}
    }
  }
