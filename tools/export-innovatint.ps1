...
$bases = Query-Rows "SELECT BASEID,PRODUCTID,ABASEID,CASE UPPER(TRIM(BASECODE)) WHEN 'HI WHITE' THEN 'H' WHEN 'MEDIUM' THEN 'M' WHEN 'DEEP' THEN 'D' WHEN 'EXTRA DEEP' THEN 'XD' WHEN 'TRANSPARENT' THEN 'T' WHEN 'YELLOW' THEN 'Y' WHEN 'PASTEL' THEN 'P' WHEN 'WHITE ASP' THEN 'W' WHEN 'BLUE BC' THEN 'BC' WHEN 'BLUE TC' THEN 'TC' ELSE BASECODE END AS BASECODE,IFNULL(NOMINALFILL,''),STRENGTH FROM basepaint ORDER BY BASEID" @("baseId","productId","aBaseId","baseCode","nominalFill","strength")
