...
function workDescription(block) {
  const match=String(block||"").match(/Pos\s+Menge\s+Einh\.?\s+Leistung\s*\n([\s\S]*?)(?=\n[\d.,]+\s+Std\.)/i);
  if(!match)return "";
  return match[1].split("\n").map(x=>x.replace(/\s+/g," ").trim()).filter(x=>x&&!/^Stunden$/i.test(x)).join(" · ").slice(0,2000);
}
