...
def install(ns):
    ...
    if isinstance(allowed, set):
        allowed.update({"/incoming/revolut", "/incoming/revolut/candidates"})
...
function assignInvoice(source,id,name){if(!confirm(`${name}\n\nDiese Rechnung mit der Revolut-Buchung bezahlen und zuordnen?`))return;post('supplier_payment',source,id,'Revolut manuell zugeordnet').catch(e=>msg.textContent=e.message)}document.getElementById('bookNoReceipt').onclick=()=>{const cat=document.getElementById('category').value,note=document.getElementById('note').value.trim();if(!confirm('Diese Zahlung ohne Rechnung vollständig buchen?'))return;post(cat,'','',note||'Revolut: Zahlung ohne Beleg').catch(e=>msg.textContent=e.message)};search.oninput=renderCandidates;document.getElementById('close').onclick=()=>dlg.close();load().catch(e=>rows.innerHTML=`<div class="empty">${esc(e.message)}</div>`);
})();
</script></body></html>'''
