    ...
    if "brain_revolut_business_sync" not in app.view_functions:
        ...
        def brain_revolut_business_sync():
            try:
                ...
                today = date.today()
                start = date.fromisoformat(str(body.get("from") or date(today.year, 1, 1).isoformat()))
...
async function load(){const [sr,cr]=await Promise.all([fetch('/incoming/reconciliation/statements',{cache:'no-store'}),fetch('/incoming/revolut/candidates',{cache:'no-store'})]),s=await sr.json(),c=await cr.json();if(!sr.ok||!s.ok)throw Error(s.error||'Buchungen nicht erreichbar');if(!cr.ok||!c.ok)throw Error(c.error||'Belege nicht erreichbar');candidates=c.items||[];document.getElementById('candidateCount').textContent=candidates.length;const statements=(s.statements||[]).filter(x=>String(x.source||'').startsWith('REVOLUT')),moves=statements.flatMap(x=>x.movements||[]),open=moves.filter(x=>x.status!=='reconciled'&&Number(x.remaining)>.005);document.getElementById('openCount').textContent=open.length;document.getElementById('openTotal').textContent=money(open.reduce((a,x)=>a+Number(x.remaining||0),0));rows.innerHTML=statements.length?statements.map(st=>`<section class="statement"><div class="statement-head"><div><strong>${esc(st.source==='REVOLUT_BUSINESS'?'Revolut Business':'Revolut')} · ${esc(st.accountIban||'')}</strong><div class="sub">${esc(date(st.periodStart))} – ${esc(date(st.periodEnd))} · ${st.movementCount} Buchungen</div></div><span class="${st.status==='complete'?'ok':'rest'}">${st.status==='complete'?'✓ vollständig zugeordnet':'Rest '+esc(money(st.remaining,st.currency))}</span></div>${(st.movements||[]).map(x=>`<div class="move ${esc(x.status)}"><div>${esc(date(x.bookingDate))}</div><div><strong>${esc(x.counterpartyName||'Ohne Gegenpartei')}</strong><div class="sub">${esc(x.reference||x.endToEndId||'')}</div>${x.suggestedCategory?`<div class="suggestion">Vorschlag: passende Rechnung gefunden</div>`:''}</div><div><div class="amount">${x.direction==='out'?'−':'+'}${esc(money(x.amount,x.currency))}</div><div class="rest">Rest ${esc(money(x.remaining,x.currency))}</div></div><div>${x.status==='reconciled'||Number(x.remaining)<=.005?'<span class="badge business">✓ Zugeordnet</span>':`<button class="primary" data-move="${x.id}" data-direction="${esc(x.direction)}" data-amount="${Number(x.remaining||x.amount)}" data-currency="${esc(x.currency)}" data-title="${esc(x.counterpartyName||'Buchung')}" data-suggested-source="${esc(x.suggestedTargetSource||'')}" data-suggested-id="${esc(x.suggestedTargetId||'')}">Zuordnen</button>`}</div></div>`).join('')}</section>`).join(''):'<div class="empty">Noch keine Revolut-Buchungen importiert.</div>';rows.querySelectorAll('[data-move]').forEach(b=>b.onclick=()=>openAssign(b))}
...
async function sync(){const button=document.getElementById('sync'),status=document.getElementById('syncMsg');button.disabled=true;status.textContent='Revolut Business wird live abgerufen …';try{const r=await fetch('/incoming/revolut/sync',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}),d=await r.json();if(!r.ok||!d.ok)throw Error(d.error||'Live-Abruf fehlgeschlagen');status.textContent=`Live aktualisiert · ${d.added||0} neue Buchung(en) · ${d.attachmentsQueued||0} Beleg(e) übernommen${(d.receiptErrors||[]).length?' · '+d.receiptErrors.length+' Beleg(e) nicht abrufbar':''}`}finally{button.disabled=false}}
function assignInvoice(source,id,name){if(!confirm(`${name}\n\nDiese Rechnung mit der Revolut-Buchung bezahlen und zuordnen?`))return;post('supplier_payment',source,id,'Revolut manuell zugeordnet').catch(e=>msg.textContent=e.message)}document.getElementById('bookNoReceipt').onclick=()=>{const cat=document.getElementById('category').value,note=document.getElementById('note').value.trim();if(!confirm('Diese Zahlung ohne Rechnung vollständig buchen?'))return;post(cat,'','',note||'Revolut: Zahlung ohne Beleg').catch(e=>msg.textContent=e.message)};search.oninput=renderCandidates;document.getElementById('close').onclick=()=>dlg.close();document.getElementById('sync').onclick=()=>sync().then(load).catch(e=>{document.getElementById('syncMsg').textContent=e.message;return load()});sync().then(load).catch(e=>{document.getElementById('syncMsg').textContent=e.message;load().catch(x=>rows.innerHTML=`<div class="empty">${esc(x.message)}</div>`) });
})();
</script></body></html>'''
