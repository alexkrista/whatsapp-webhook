...
const job=()=>({jobId:"26101",name:"Musterauftrag",street:"Baustellenweg",houseNumber:"5",postalCode:"6800",city:"Feldkirch",wwAddressId:"4711",wwCustomerNumber:"1200",customerMaster:{wwAddressId:"4711",email:"person@example.at"},projectContacts:{owner:{ownerRole:"Firma",customer:"Beispiel",womanTitle:"Firma",womanLastName:"Beispiel",manLastName:"Beispiel",womanEmail:"person@example.at",phoneOwnerMan:"05555 123"},siteManager:{},architect:{}}});
test("Firmenmaske hat optionale Personen; Übernahme und Speichern bewahren Mail, WW-Link und Quelle",async t=>{
  const requests=[],company={legalName:"Beispiel GmbH",street:"Firmenweg",houseNumber:"24",postalCode:"6890",city:"Lustenau",uid:"ATU12345678",email:"office@example.at",phone:"05555 999",website:"https://example.at/",sourceUrl:"https://example.at/impressum",sourceCheckedAt:"2026-09-19T20:00:00Z"};
  ...
  await d.getElementById("bkOwnerLookupWeb").onclick();assert.equal(requests[0].body.query,"Beispiel");
  d.querySelector("[data-company-index]").click();assert.equal(d.getElementById("bkOwnerCustomer").value,"Beispiel GmbH");assert.equal(d.getElementById("bkOwnerWomanEmail").value,"person@example.at");assert.equal(d.getElementById("bkOwnerWomanPhone").value,"05555 123");assert.equal(d.getElementById("bkHomeCity").value,"Lustenau");
  ...
  assert.equal(saved.wwAddressId,"4711");assert.equal(saved.contactName,"Beispiel GmbH");assert.equal(saved.contactEmail,"person@example.at");assert.equal(saved.projectContacts.owner.manLastName,"");assert.equal(saved.projectContacts.owner.womanFirstName,"Eva");assert.equal(saved.projectContacts.owner.uid,"ATU12345678");assert.equal(saved.customerMaster.uid,"ATU12345678");assert.equal(saved.projectContacts.owner.sourceUrl,company.sourceUrl);assert.equal(saved.street,"Baustellenweg");
...
test("Angebot und AB nennen bei Firmen die Firma vor optionalen Ansprechpartnern",()=>{
  ...
  assert.deepEqual(Array.from(context.offerRecipientNames({ownerRole:"Firma",customer:"Beispiel GmbH",womanFirstName:"Eva",womanLastName:"Muster"},{})),["Beispiel GmbH","Eva Muster"]);
