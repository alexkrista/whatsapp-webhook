...
assert.match(access,/@media\(max-width:900px\)[\s\S]*?\.krista-shell-topbar\{position:relative!important;top:auto!important\}/);
assert.match(access,/\.krista-access-slot\{[\s\S]*?grid-row:2!important;[\s\S]*?grid-template-columns:repeat\(3,minmax\(0,1fr\)\)!important/);
assert.match(access,/\.krista-door-row\{[\s\S]*?grid-row:2!important;[\s\S]*?grid-template-columns:repeat\(3,minmax\(0,1fr\)\)!important/);
