...
for(const text of ["Unterschriebene Regiebericht-PDFs","Kurzübersicht Regieberichte","Mitarbeiter","Arbeitsstunden","Material gesamt"])assert.ok(ui.includes(text),`Regie-Import fehlt: ${text}`);assert.match(server,/documentation\/regie-report/);assert.match(server,/regie_report_imported/);
...
for(const text of ["Rapportstunden","Stundenwert gesamt","Arbeitsstunden","Stunden EUR","Material EUR","Gesamtsumme"])assert.ok(ui.includes(text),`Regiesummen fehlen: ${text}`);
