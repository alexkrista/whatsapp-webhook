...
(async () => {
  ...
  try {
    ...
    const nextAId = await invoke(routes["get:/admin/api/materials/next-id"], { query: { prefix: "A" } });
    assert.equal(nextAId.body.materialId, "A01", "Das kleinste freie Buchstaben-Kürzel wird vorgeschlagen");
    const nextLgId = await invoke(routes["get:/admin/api/materials/next-id"], { query: { prefix: "LG" } });
    assert.equal(nextLgId.body.materialId, "LG 01", "Mehrbuchstaben-Kürzel erhalten eine gut lesbare Nummer");
    ...
    const withoutSupplier = await invoke(routes["get:/admin/api/materials"], { query: { supplier: "Ohne Lieferant", limit: "5000" } });
    assert.equal(withoutSupplier.statusCode, 200);
    assert(withoutSupplier.body.materials.length > 0, "Artikel ohne Lieferant werden beim Sonderfilter angezeigt");
    assert(withoutSupplier.body.materials.every(row => !row.supplier), "Sonderfilter zeigt nur Artikel ohne Lieferant");
