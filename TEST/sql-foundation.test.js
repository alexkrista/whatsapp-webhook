...
assert.match(views, /WHEN f\.status IN \('Auftrag', 'Laufend'\) THEN GREATEST\(f\.total_target_minutes - f\.actual_total_minutes, 0\)/);
...
assert.match(views, /CREATE OR REPLACE VIEW kristine\.portfolio_hour_kpi_candidates_v1/);

assert.match(controls, /FROM kristine\.portfolio_hour_rows_v1/);
assert.match(controls, /HAVING COUNT\(\*\) > 1/);

function committedMinutes(status, target, actual) {
  if (status === "Auftrag") return target;
  if (status === "Laufend") return Math.max(0, target - actual);
  return 0;
}

function remainingActiveMinutes(status, target, actual) {
  return ["Auftrag", "Laufend"].includes(status) ? Math.max(0, target - actual) : 0;
}

assert.equal(committedMinutes("Auftrag", 600, 120), 600);
assert.equal(committedMinutes("Laufend", 600, 120), 480);
assert.equal(remainingActiveMinutes("Auftrag", 600, 120), 480);
assert.equal(remainingActiveMinutes("Laufend", 600, 720), 0);
assert.equal(remainingActiveMinutes("Fertig – nicht abgerechnet", 600, 120), 0);
