...
assert.match(topbar,/baustellen-ww-import\.js\?v=20260913-hours-cache-1/);
assert.match(ui,/Mit vorhandener Akte verbinden/);
assert.match(ui,/Weitere Akte verbinden/);
...
assert.match(ui,/\/admin\/api\/job\/\$\{encodeURIComponent\(targetJobId\)\}\/collection/);
assert.doesNotMatch(ui,/\/merge/);
assert.match(ui,/ZUSAMMEN/);
assert.match(ui,/Nicht destruktiv/);
assert.match(ui,/Sammelmappe bilden/);
for(const text of ["WinWorker-Aufträge übernehmen","Aus WW übernehmen","Als Einzelakten übernehmen","bereits da","wwProjectIndex","wwProjectNumber"]){
...
assert.match(ui,/searchMode="all"/);
assert.match(brain,/"hidden": is_hidden/);
assert.match(ui,/function visibleRows\(\)/);
