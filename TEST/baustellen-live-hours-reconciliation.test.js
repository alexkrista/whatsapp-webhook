...
assert.match(ui,/window\.BaustellenSources\.ww\("hours"/);
assert.match(sources,/if\(host===LOCAL\)\{if\(token\)headers\["X-Krista-Token"\]=token\}/);
assert.match(sources,/X-Krista-Brain-Permit/);
...
assert.match(ui,/collectionJobIds\(j\)\.reduce\(\(sum,id\)=>sum\+num\(liveByJob\.get\(id\)\?\.totalHours\),0\)/);
...
assert.match(ui,/rawExcluded=selectedExclusions/);
assert.match(ui,/kristineHours=kristineTotal/);
