...
for(const text of ["Kalkulation / Angebot","Regie + Material","Innenmalerei","Abdeckarbeiten – einmalig auswählen","Boden und Laufwege","Fassade","Fassadenfarbe 2x","Bad komplett","Fugenloses Bad","Terra Icon Unikatbelag","Kalkputz","Sumpfkalk-Wohlfühlputz","WDVS kurz","Isolierung kleben","Aufmaß genau","Überschlag: Boden × 3,5","Leistungsumfang / Räume","Mengen und Einheiten"])assert.ok(ui.includes(text),`UI enthält ${text}`);
for(const text of ["offer-draft","coverSteps","bathroomSteps","bathroomCompactSteps","limeSteps","wdvsSteps","bathroomCalc","measurement","scopeDescription","showQuantities"])assert.ok(server.includes(text),`Server enthält ${text}`);
