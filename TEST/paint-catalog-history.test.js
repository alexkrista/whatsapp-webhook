...
(async()=>{
 ...
 try{
 const get=async p=>(await fetch(base+p,{headers:{'x-admin-token':'test-only'}})).json();
  ...
  assert.equal((await fetch(base+'/admin/api/paint/recipe?colourId=1&productId=11&canSizeId=2',{headers:{'x-admin-token':'test-only'}})).status,400);
  ...
  const p=await get('/admin/api/paint/ean-reconcile');const changed=await fetch(base+'/admin/api/paint/ean-reconcile',{method:'POST',headers:{'content-type':'application/json','x-admin-token':'test-only'},body:JSON.stringify({revision:p.revision})});assert.equal(changed.status,200);assert.equal((await get('/admin/api/paint/ean-reconcile')).changes.length,0);
  assert.equal((await fetch(base+'/admin/api/paint/ean-reconcile',{method:'POST',headers:{'content-type':'application/json','x-admin-token':'test-only'},body:JSON.stringify({revision:p.revision})})).status,409);
