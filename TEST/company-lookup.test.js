...
const company={legalName:"Beispiel GmbH",street:"Marktstraße",houseNumber:"3",postalCode:"6800",city:"Feldkirch",country:"Österreich",uid:"ATU 12345678",email:"Office@Beispiel.at",phone:"+43 5555 123",website:"https://beispiel.at/",sourceUrl:"https://beispiel.at/impressum"};
...
test("Firmensuche liefert belegte öffentliche Daten und verwirft unbelegte oder fremde Quellen",()=>{
  ...
  assert.equal(result.length,1);assert.equal(result[0].uid,"ATU12345678");assert.equal(result[0].email,"office@beispiel.at");
...
test("Websuche wird erzwungen; gleiche Suche wiederverwendet statt doppelt berechnet",async()=>{
  let calls=0;
  const lookup=createCompanyLookup({apiKey:"test-only",fetchImpl:async(url,options)=>{calls++;assert.equal(url,"https://api.openai.com/v1/responses");const request=JSON.parse(options.body);assert.equal(request.store,false);assert.equal(request.tool_choice,"required");assert.equal(request.tools[0].type,"web_search");assert.equal(JSON.parse(request.input).company,"Beispiel");return {ok:true,json:async()=>response([company])}}});
  const [a,b]=await Promise.all([lookup("Beispiel","Feldkirch"),lookup("Beispiel","Feldkirch")]);assert.deepEqual(a,b);await lookup("Beispiel","Feldkirch");assert.equal(calls,1);
});
