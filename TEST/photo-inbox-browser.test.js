...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 await page.goto('http://inbox.test/?photoGroup=g1');await page.locator('[data-group]').waitFor();assert.equal(await page.locator('[data-job]').first().inputValue(),'26091 · Baustelle B');
 assert.equal(await page.getByText('Bisherige Baustelle: 26091 · zur Nachprüfung').count(),2);await page.getByText('Fehlende Dateien anzeigen').click();await page.getByText('missing.jpg',{exact:true}).waitFor();assert.equal(await page.locator('[data-job]').nth(1).inputValue(),'26091 · Baustelle B');await page.getByText('Stempelvorschlag: 26080 · Baustelle A · Stempelung um 07:00').waitFor();await page.locator('[data-job]').nth(1).fill('');await page.getByRole('button',{name:'Zuordnungen bestätigen'}).click();assert.equal(changes,null);await page.getByText('Bitte für jedes Foto eine Baustelle wählen.').waitFor();
 ...
 assert.equal(await page.locator('[data-job]').nth(1).inputValue(),'26091 · Baustelle B');await page.getByRole('button',{name:'Zuordnungen bestätigen'}).click();await page.getByText('Keine Fotos zur Zuordnung offen.').waitFor();
