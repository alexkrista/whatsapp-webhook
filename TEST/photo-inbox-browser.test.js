...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 if(u.pathname==='/kristine/api/photo-inbox')return route.fulfill({json:{historyImport:{added:2,reopened:0,alreadyPending:0,missing:['missing.jpg']},items:confirmed?[]:[0,1].map(i=>({retrospective:true,previousJobId:'26091',groupId:'g1',file:'photo'+i+'.jpg',url:'/image.jpg',employeeName:'Clemens',date:'2026-09-12',at:'10:00',category:'photo',suggestion:{jobId:i?'':'26080',reason:i?'Keine Stempelung':'Stempelung um 07:00'}}))}});
 ...
 assert.equal(await page.getByText('Bisherige Baustelle: 26091 · zur Nachprüfung').count(),2);await page.getByText('Fehlende Dateien anzeigen').click();await page.getByText('missing.jpg',{exact:true}).waitFor();await page.getByRole('button',{name:'Zuordnungen bestätigen'}).click();assert.equal(changes,null);await page.getByText('Bitte für jedes Foto eine Baustelle wählen.').waitFor();
