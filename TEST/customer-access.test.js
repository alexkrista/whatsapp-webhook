...
async function fixture(t,withOffice=false,overrides={}){
 ...
 let members=["24177",...Array.from({length:38},(_,i)=>String(26001+i))],time=Date.now(),beforePdf=null,documentation={};const customerNotices=[],reopenedNotices=[],portalInvitations=[],acceptedOffers=[];
 ...
 return {dir,write,metas,request,invite,login,access,customerNotices,reopenedNotices,portalInvitations,acceptedOffers,advance:ms=>time+=ms,setMembers:ids=>members=ids,setBeforePdf:fn=>beforePdf=fn,setDocumentation:(id,rows)=>{documentation[id]=rows}};
}
...
test("a finalized offer is visible and commissionable through an existing normal portal link",async t=>{
 ...
 const invalidDate=await f.request("/kundenportal/api/offer/accept",{method:"POST",headers:{...headers,"x-csrf-token":project.csrf},body:JSON.stringify({confirmed:true,termsAccepted:true,termsVersion:project.offer.terms.version,paymentTerm:"net14",preferredDate:"2026-13-40"})});assert.equal(invalidDate.status,400);
 const accepted=await f.request("/kundenportal/api/offer/accept",{method:"POST",headers:{...headers,"x-csrf-token":project.csrf},body:JSON.stringify({confirmed:true,termsAccepted:true,termsVersion:project.offer.terms.version,paymentTerm:"skonto5_2",preferredDate:"2026-10-10"})}),acceptedText=await accepted.text();assert.equal(accepted.status,200,acceptedText);const acceptedBody=JSON.parse(acceptedText);assert.equal(acceptedBody.orderCreated,true);assert.equal(acceptedBody.orderSchedule.status,"requested");assert.equal(f.acceptedOffers.length,1);assert.equal(f.acceptedOffers[0].acceptance.paymentTerm,"skonto5_2");assert.equal(f.acceptedOffers[0].acceptance.preferredDate,"2026-10-10");
...
test("customer sees an open alternative appointment and can confirm it once",async t=>{
 let schedule={status:"proposed",requestedDate:"2026-10-05",proposedDate:"2026-10-07",proposedFrom:"08:00",proposedTo:"16:00",updatedAt:"2026-09-18T10:00:00.000Z"},responses=[];
 const f=await fixture(t,false,{readOrderSchedule:async()=>schedule,respondToOrderScheduleProposal:async payload=>{responses.push(payload);schedule={...schedule,status:"confirmed",confirmedDate:schedule.proposedDate,confirmedFrom:schedule.proposedFrom,confirmedTo:schedule.proposedTo,customerConfirmedAt:"2026-09-18T11:00:00.000Z"};return schedule;}}),cookie=await f.login((await f.invite()).portalUrl),project=await(await f.request("/kundenportal/api/project",{headers:{Cookie:cookie}})).json();
 assert.equal(project.orderSchedule.status,"proposed");assert.equal(project.orderSchedule.canRespond,true);
 const response=await f.request("/kundenportal/api/order-schedule/response",{method:"POST",headers:{Cookie:cookie,"x-csrf-token":project.csrf},body:JSON.stringify({confirmed:true})});assert.equal(response.status,200);const body=await response.json();assert.equal(body.orderSchedule.status,"confirmed");assert.equal(responses.length,1);assert.equal(responses[0].jobId,"24177");
 assert.equal((await f.request("/kundenportal/api/order-schedule/response",{method:"POST",headers:{Cookie:cookie},body:JSON.stringify({confirmed:true})})).status,403);
});
...
test("Bauherr, Architekt und Bauleitung erhalten getrennte persönliche Links mit eigenem Lesestatus",async t=>{
 const f=await fixture(t),portal=f.metas["24177"].customerPortal;
 portal.recipients=[
  {id:"owner",role:"owner",roleLabel:"Bauherrschaft",name:"Egon Bauherr",phone:"+43664111",email:""},
  {id:"architect",role:"architect",roleLabel:"Architekt",name:"Anna Architektur",phone:"",email:"architekt@example.at"},
 {id:"siteManager",role:"siteManager",roleLabel:"Bauleitung",name:"Berta Bauleitung",phone:"+43664222",email:""},
 ];portal.selectedRecipientIds=["owner","architect","siteManager"];
 const pointResponse=await f.request("/admin/api/job/24177/customer-points",{method:"POST",headers:{"x-test-admin":"yes"},body:JSON.stringify({title:"Farbton abstimmen",text:"Bitte den Farbton gemeinsam bestätigen.",sendNow:true})});assert.equal(pointResponse.status,201);const sharedPoint=(await pointResponse.json()).point;
 const response=await f.request("/admin/api/job/24177/customer-portal/invitations/send",{method:"POST",headers:{"x-test-admin":"yes"},body:JSON.stringify({recipientIds:portal.selectedRecipientIds})});
 assert.equal(response.status,200);const data=await response.json();assert.equal(data.results.length,3);assert.equal(new Set(data.results.map(row=>row.portalUrl)).size,3);assert.equal(f.portalInvitations.length,3);assert(data.results.every(row=>row.sent));
 const architect=data.results.find(row=>row.recipient.id==="architect"),cookie=await f.login(architect.portalUrl),project=await(await f.request("/kundenportal/api/project",{headers:{Cookie:cookie}})).json();assert.equal(project.customerName,"Anna Architektur");assert.equal(project.recipient.role,"architect");assert(project.points.some(point=>point.id===sharedPoint.id));
 const statuses=f.access.listInvitations("24177"),read=statuses.find(row=>row.recipientId==="architect");assert(read.sentAt);assert(read.readAt);assert.equal(statuses.filter(row=>["owner","architect","siteManager"].includes(row.recipientId)).length,3);
});
...
test("points keep titles and every office task transition while the customer is offline, including after restart and stale omission",async t=>{
 ...
 const save=async tasks=>{const response=await f.request("/kristine/api/tasks",{method:"PUT",headers:{"x-test-admin":"yes"},body:JSON.stringify({tasks})});assert.equal(response.status,200);return response.json();};
 let saved=await save(tasks.map(task=>({...task,status:"done",completedAt:new Date().toISOString(),reminder:"DO_NOT_EXPOSE_INTERNAL_COMMENT"})));tasks=saved.tasks;
 saved=await save(tasks.map(task=>({...task,status:"open",completedAt:null})));tasks=saved.tasks;
 saved=await save(tasks.map(task=>({...task,status:"done",completedAt:new Date().toISOString()})));tasks=saved.tasks;
...
test("office points can be captured privately, sent with a direct link and followed through customer confirmation",async t=>{
 const f=await fixture(t,true,{notifyOnCompletion:true}),admin={"x-test-admin":"yes"};
 let response=await f.request("/admin/api/job/24177/customer-points",{method:"POST",headers:admin,body:JSON.stringify({title:"Silikonfuge Dusche",area:"Bad OG",text:"Fuge beim Ablauf sauber nachziehen.",assigneeId:"mario",dueDate:"2026-09-22",sendNow:false})});
 assert.equal(response.status,201);let created=await response.json();assert.equal(created.point.state,"assigned");assert.equal(created.point.visibility,"internal");assert.equal(created.task.assigneeName,"Mario");
 const cookie=await f.login((await f.invite()).portalUrl),projectHeaders={Cookie:cookie};let project=await(await f.request("/kundenportal/api/project",{headers:projectHeaders})).json();assert(!project.points.some(row=>row.id===created.point.id));
 response=await f.request(`/admin/api/job/24177/customer-points/${created.point.id}/send`,{method:"POST",headers:admin,body:"{}"});assert.equal(response.status,200);const sent=await response.json();assert.equal(sent.notification.sent,true);assert.equal(f.customerNotices.length,1);assert.match(f.customerNotices[0].portalUrl,new RegExp(`punkt=${created.point.id}$`));
 project=await(await f.request("/kundenportal/api/project",{headers:projectHeaders})).json();let point=project.points.find(row=>row.id===created.point.id);assert.equal(point.state,"sent");assert(point.history.some(event=>event.kind==="captured"));assert(point.history.some(event=>event.kind==="assigned"));assert(point.history.some(event=>event.kind==="sent"));
 let tasks=mergePortalTasks(f.dir,[]),task=tasks.find(row=>row.customerPointId===point.id),save=async rows=>{const result=await f.request("/kristine/api/tasks",{method:"PUT",headers:admin,body:JSON.stringify({tasks:rows})});assert.equal(result.status,200);return result.json();};
 const completed=await save(tasks.map(row=>row.id===task.id?{...row,status:"done",completedAt:"2026-09-22T15:00:00.000Z"}:row)),change=completed.customerPointChanges.find(row=>row.taskId===task.id);assert.equal(change.type,"internal_done");assert.equal(change.notification.sent,true);assert.equal(f.customerNotices.length,2);assert.equal(f.customerNotices[1].reason,"confirmation");
 project=await(await f.request("/kundenportal/api/project",{headers:projectHeaders})).json();point=project.points.find(row=>row.id===point.id);assert.equal(point.state,"awaiting_confirmation");assert.equal(point.canConfirm,true);const staleDone=structuredClone(mergePortalTasks(f.dir,[]).find(row=>row.customerPointId===point.id));
 const csrfHeaders={...projectHeaders,"x-csrf-token":project.csrf};response=await f.request(`/kundenportal/api/point/${point.id}/read`,{method:"POST",headers:csrfHeaders,body:"{}"});assert.equal(response.status,200);point=(await response.json()).point;assert.equal(point.state,"awaiting_confirmation");assert.equal(point.history.at(-1).kind,"read");
 response=await f.request(`/kundenportal/api/point/${point.id}/confirmation`,{method:"POST",headers:csrfHeaders,body:JSON.stringify({confirmed:false,comment:"Beim Ablauf fehlt noch ein kleines Stück."})});assert.equal(response.status,200);point=(await response.json()).point;assert.equal(point.state,"reopened");assert.equal(point.status,"open");assert.equal(point.history.at(-1).comment,"Beim Ablauf fehlt noch ein kleines Stück.");assert.equal(f.reopenedNotices.length,1);
 let staleResult=await save([staleDone]);assert(staleResult.customerPointChanges.some(row=>row.type==="stale_status_ignored"));assert.equal(f.customerNotices.length,2);
 tasks=mergePortalTasks(f.dir,[]);task=tasks.find(row=>row.customerPointId===point.id);assert.equal(task.status,"open");assert.equal(task.customerPointResponse,"Beim Ablauf fehlt noch ein kleines Stück.");
 await save(tasks.map(row=>row.id===task.id?{...row,status:"done",completedAt:"2026-09-23T09:00:00.000Z"}:row));project=await(await f.request("/kundenportal/api/project",{headers:projectHeaders})).json();point=project.points.find(row=>row.id===point.id);assert.equal(point.state,"awaiting_confirmation");
 response=await f.request(`/kundenportal/api/point/${point.id}/confirmation`,{method:"POST",headers:{...projectHeaders,"x-csrf-token":project.csrf},body:JSON.stringify({confirmed:true})});assert.equal(response.status,200);point=(await response.json()).point;assert.equal(point.state,"confirmed");assert.equal(point.status,"done");assert.equal(point.history.at(-1).kind,"customer_confirmed");
});
