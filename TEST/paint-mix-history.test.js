...
(async()=>{
 ...
 const project=()=>call('/admin/api/paint/mix-history/:id/resolve',{resolution:'project',jobId:'26083',jobName:'Testbaustelle',component:'Decke'},{id:'project-1'});
 ...
 const stockBefore=await fs.readFile(path.join(root,'articles.json'),'utf8'), movementsBefore=await fs.readFile(path.join(root,'movements.jsonl'),'utf8');
 await call('/admin/api/paint/bridge/history',{rows:[{...row,id:'old-1',colourCode:'Old tone',productName:'Discontinued'}],baseline:true,createTasks:false});
 const archive=jobId=>call('/admin/api/paint/mix-history/:id/resolve',{resolution:'project',jobId,jobName:'Archive',component:'Fassade'},{id:'old-1'});
 const archived=await Promise.all([archive('26083'),archive('26083')]);assert(archived.every(r=>r.statusCode===200));
 assert.equal((await archive('26084')).statusCode,409);
 assert.equal((await resolve('old-1')).statusCode,409);
 assert.equal(await fs.readFile(path.join(root,'articles.json'),'utf8'),stockBefore);
 assert.equal(await fs.readFile(path.join(root,'movements.jsonl'),'utf8'),movementsBefore);
 const archivedMaterials=(await fs.readFile(path.join(root,'job-materials.jsonl'),'utf8')).trim().split('\n').map(JSON.parse).filter(x=>x.historyId==='old-1');assert.equal(archivedMaterials.length,1);assert.equal(archivedMaterials[0].knowledgeOnly,true);assert.equal(archivedMaterials[0].component,'Fassade');assert.equal(JSON.parse(materials[0]).component,'Decke');
 const currentArticles=JSON.parse(await fs.readFile(path.join(root,'articles.json'),'utf8'));currentArticles.push({id:'IM',product:'Intelligent Matt',baseCode:'T',size:'2.5 L',stock:3,ean:'5050173175338'});await fs.writeFile(path.join(root,'articles.json'),JSON.stringify(currentArticles));
