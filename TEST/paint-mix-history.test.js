...
(async()=>{
 ...
 const archive=jobId=>call('/admin/api/paint/mix-history/:id/resolve',{resolution:'project',jobId,jobName:'Archive'},{id:'old-1'});
 ...
 const archivedMaterials=(await fs.readFile(path.join(root,'job-materials.jsonl'),'utf8')).trim().split('\n').map(JSON.parse).filter(x=>x.historyId==='old-1');assert.equal(archivedMaterials.length,1);assert.equal(archivedMaterials[0].knowledgeOnly,true);
